/**
 * @zakkster/lite-signal v1.9.0-spike (RESEARCH BUILD -- do not publish)
 * Copyright (c) Zahary Shinikchiev <shinikchiev@yahoo.com>
 * MIT License
 * --------------------
 * 1.9.0-spike -- per-edge versioning (Reflex-study pattern D), built on the
 * final 1.8.0. Staleness becomes a per-edge equality test: every
 * ReactiveLink carries `version` = the source version its target OBSERVED at
 * last read, stamped in the read cursor paths (post-pull for computed
 * sources -- the edge must record the version of the value actually
 * returned, or the reader eats a spurious recompute and the
 * one-recompute-per-node burst structure breaks). The dep walks in
 * pullComputed / executeEffect compare link.version !== dep.version instead
 * of the global-clock subtraction against evalVersion.
 *
 * DELIBERATELY KEPT: globalVersion, the evalVersion===globalVersion memo
 * fast-path, and the markEpoch clean short-circuit. The mark phase needs a
 * monotonic per-propagation stamp regardless, so "remove the global write"
 * is not a real win on this architecture; the spike isolates ONE variable --
 * the cost of edge stamping (+1 store per tracked cursor-hit read, +1 per
 * fresh link) plus equality-vs-subtraction in the walks. Spike numbers only;
 * per roadmap v3 this pattern is not locked in either direction until the
 * cold-process A/B and VersionMatrix gate say so.
 *
...
class ReactiveLink {
    constructor() {
        ...
        /** 1.9.0-spike: version of `source` OBSERVED by `target` through this
         *  edge at its last read. Staleness becomes a per-edge equality test
         *  (link.version !== source.version) instead of the global-clock
         *  subtraction against target.evalVersion. */
        this.version = 0;
    }
}
...
export function createRegistry(config) {
    ...
    function allocateLink(source, target) {
        ...
        // 1.9.0: return the existing link so the caller can re-stamp it
        // post-pull (matters when the same source is read twice in one pass
        // with an inner write in between).
        ...
        // 1.9.0: stamp what the reader observes NOW. Exact for signals (their
        // version cannot move during a read); computed readers overwrite this
        // with a post-pull stamp -- see the computed read closure.
        ...
        if (node.evalVersion !== 0) {
            ...
            while (link !== null) {
                ...
                if ((dep.flags & FLAG_COMPUTED) !== 0) {
                    // 1.9.0: in-flight dep -> force the run path (see the
                    // pullComputed walk for the full rationale).
                    if ((dep.flags & FLAG_COMPUTING) !== 0) {
                        needsRun = true;
                        break;
                    }
                    pullComputed(dep);
                }
                // 1.9.0-spike: per-edge staleness -- did THIS edge's source
                // move past what this effect observed last run?
    ...
    function pullComputed(node) {
        ...
        if (!shouldRun) {
            ...
            while (link !== null) {
                const dep = link.source;
                if ((dep.flags & FLAG_COMPUTED) !== 0) {
                    // 1.9.0: an IN-FLIGHT dep means this walk is inside that
                    // dep's own call tree -- a true cycle. Its edges may
                    // carry fresh in-read stamps (edges advance during the
                    // run, unlike evalVersion which commits after), so the
                    // equality test below would call it clean and hand back
                    // a stale cache where the clock scheme raised
                    // CycleError. Force the run path; the FLAG_COMPUTING
                    // check there restores the exact old detection.
                    if ((dep.flags & FLAG_COMPUTING) !== 0) {
                        shouldRun = true;
                        break;
                    }
                ...
                // 1.9.0-spike: per-edge staleness (equality, not clock
                // subtraction). Equality-blocked upstream recomputes keep
                // dep.version stable, so a matching edge stays clean --
                // identical propagation-blocking to the clock scheme.
                if (link.version !== dep.version) {
                    shouldRun = true;
                    break;
                }
        ...
        const read = () => {
            ...
            if (isTrackingDeps && currentObserver !== null) {
                ...
                if (expected !== null && expected.source === node) {
                    ...
                    // 1.9.0: +1 store per tracked cursor-hit read -- THE cost
                    // this spike measures. A signal's version cannot move
                    // during its own read, so this stamp is exact.
                    expected.version = node.version;
                } else {
    ...
    function computed(fn, opts) {
        ...
        const read = () => {
            ...
            if (isTrackingDeps && currentObserver !== null) {
                ...
                if (expected !== null && expected.source === node) {
                    activeObserverCurrentDep = expected.nextDep;
                    // 1.9.0: stamp AFTER the pull -- the pull may recompute
                    // and bump node.version, and the edge must record the
                    // version of the value actually returned. A pre-pull
                    // stamp would leave the edge stale and buy a spurious
                    // recompute of the reader on its next pull, breaking the
                    // one-recompute-per-node burst structure.
                    const v = pullComputed(node);
                    expected.version = node.version;
                    return v;
                }
                const link = allocateLink(node, currentObserver);
                const v = pullComputed(node);
                if (link !== null) link.version = node.version;
                return v;
            }
            return pullComputed(node);
        };
    ...
    function boxGet() {
        ...
        if (isTrackingDeps && currentObserver !== null) {
            ...
            if (expected !== null && expected.source === node) {
                activeObserverCurrentDep = expected.nextDep;
                expected.version = node.version;   // 1.9.0: see signal read
            } else {
                allocateLink(node, currentObserver);
            }
        }
    ...
    function boxComputedGet() {
        ...
        if (isTrackingDeps && currentObserver !== null) {
            ...
            if (expected !== null && expected.source === node) {
                activeObserverCurrentDep = expected.nextDep;
                const v = pullComputed(node);
                expected.version = node.version;   // post-pull: see computed read
                return v;
            }
            const link = allocateLink(node, currentObserver);
            const v = pullComputed(node);
