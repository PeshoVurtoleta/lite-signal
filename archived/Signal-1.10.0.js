/**
 * @zakkster/lite-signal v1.10.0 (candidate)
 * Copyright (c) Zahary Shinikchiev <shinikchiev@yahoo.com>
 * MIT License
 * --------------------
 * 1.10.0 -- cold-path additions only: computed fn(prev) + getOwner/runWithOwner.
 *
 * VERSIONING CONTRACT (the handshake): every release is either an
 * optimization or NEW COLD-PATH FUNCTIONALITY. This release adds zero
 * instructions to any hot path -- pullComputed, markDownstream, the read
 * closures, the write closures, and executeEffect are BYTE-IDENTICAL to
 * 1.9.1 (verified by function-body diff; see RESULTS-1_10_0).
 *
 * NEW: computed((prev) => ..., {initial}) -- the body receives its previous
 * value (undefined or the {initial} seed on the first run). Implemented as
 * a wrapper closure chosen ONCE at creation from declared arity, so the
 * recompute call site never changes. Applies to computed() and
 * computedBox().
 *
 * NEW: getOwner() / runWithOwner(handle, fn) -- the async-continuation pair
 * on the existing owner/observer split. Gen-guarded handles; stale owners
 * degrade to rooted execution; tracking off inside fn, like createRoot.
 *
 * NOT in this release, by design (rejection ledger #15): upstream suite
 * #179 "computed self-increment". A computed that mutates its own tracked
 * input is a construct this engine deliberately excludes -- closing the
 * test cost +2.7-4.7% on frame medians (flags test per computed read, mark
 * compare, tail branch; no single owner, measured by 4-way bisect) and
 * changed the observable contract of a pathological shape. Predictability
 * and hot-path integrity outrank the scoreboard's last cell. The full
 * technical record (alien-contract analysis, currentObserver identity
 * finding, bisect data) is preserved in the ledger for the day the trade
 * changes.
 *
 * Original 1.9.1 header follows.
 * --------------------
 * 1.9.1 -- explicit resource management (`using` / Symbol.dispose), additive.
 *
 * NEW: [Symbol.dispose] on lifecycle objects, existence-guarded (works on
 * every Node/browser that defines the well-known symbol; `using` syntax
 * picks it up automatically where supported, manual [Symbol.dispose]()
 * works everywhere else):
 *   - createRegistry() result  -> destroy()
 *   - effect() disposer        -> itself (idempotent, gen-guarded)
 *   - createScope() disposer   -> itself
 *   - signalBox/computedBox    -> dispose(this), on the SHARED PROTOTYPES
 *     (zero per-instance creation cost)
 * Callable signal()/computed() handles are deliberately NOT stamped.
 *
 * Original 1.9.0 header follows.
 * --------------------
 * 1.9.0 -- inner-write fixed point + onSettled + link-churn counters
 * (second Reflex-study cycle; the per-edge-versioning 1.9 spike was shelved
 * with numbers as ledger #14 and lives in engines/shelved).
 *
 * FIXED (conformance): effect(() => { const v = n(); if (cond) n.set(f(v)) })
 * now iterates to a fixed point. Previously the post-body
 * `evalVersion = globalVersion` silently absorbed the effect's own write
 * (markDownstream cannot enqueue a COMPUTING effect), so the body observed
 * the pre-write value exactly once and never re-ran -- one of the known
 * upstream-conformance gaps. Mechanism adapted from Reflex's invalidateSlow
 * confirmed-prefix discipline, translated to the MARK PHASE: the enqueue
 * gate in markDownstream already visits a RUNNING effect exactly when a
 * write lands on one of its true tracked deps -- it now records that skip
 * in FLAG_SELF_DIRTY (skip path only, off the enqueue hot path). After a
 * completed body, ONE flag test decides: on a self-mark, evalVersion parks
 * at the pre-body clock snapshot and the effect self-enqueues; the ordinary
 * pre-walk drives the iteration and equality-blocked writes terminate it.
 *
 * TWO-TIER CONTRACT (validated against upstream suite #235): BARE
 * self-writes iterate; self-writes inside a batch the body ITSELF opened
 * are ATOMIC -- they coalesce into the current run and never re-trigger it.
 * A non-idempotent batched self-increment (b = b+10; b = b+1) has no fixed
 * point, so absorption is the only terminating semantics there -- matching
 * alien-signals, @vue/reactivity, and @solidjs/signals on the upstream
 * suite. Mechanically: marks landing at batchDepth greater than the depth
 * at the body's start do not set FLAG_SELF_DIRTY.
 *
 * Costs: effects that never write pay ONE flag test per run; effects
 * writing only UNTRACKED signals pay nothing at all (unrelated writes never
 * mark the effect -- measured dead-neutral cold-process); true livelocks
 * hit maxFlushPasses. Delivery of fixed-point passes follows the
 * flushStrategy contract (eager: immediate; sab: batch exit; manual:
 * explicit flush()).
 *
 * NEW: registry.onSettled(fn) / module-level onSettled(fn) -- single
 * nullable quiescence listener (mutation-hook discipline, zero cost
 * detached). Fires after a flush drains the queue to empty at batchDepth 0
 * with no buffered errors and at least one pass run. Listener runs untracked.
 *
 * NEW: stats() gains totalLinkAllocations / totalLinkDisposals (14 keys) --
 * topology-stability detector v1 from the Reflex graph-reduction study. The
 * cursor fast-path never touches link alloc/free, so zero deltas over an
 * interval mean topology-stable steady state; churn-per-recompute is the
 * gating datum for any future static-plan (pattern E) work. Cold-path SMI
 * bumps only.
 *
 * Original 1.8.0 header follows.
 *
 * --------------------
 * 1.8.0 -- effect cleanup return (Reflex-study pattern A).
 *
 ...
 * order. Non-function returns are ignored. Zero cost on bodies returning
 * undefined (one typeof check per effect run, off the mark/pull path).
 * Computeds are untouched: a computed returning a function keeps treating it
 * as the VALUE.
 *
 * REJECTED during this cycle (ledger #13, measured): the "lean node shape"
 * batch-revert side stack (Reflex pattern C). Moving the on-node
 * {preBatchValue, preBatchVersion, revertEpoch} triple into a registry-level
 * capture stack -- parallel arrays AND an interleaved stride-3 variant --
 * held the full behavioral contract (439/439, conformance clean) and was
 * VersionMatrix-neutral, but regressed capture-dense batched writes +5-9%
 * and revert-heavy batches +20% in cold-process A/B (n=3, consistent).
 * Family (c) loses in BOTH directions: these fields are not cold -- the
 * first batched write is a hot edge, and the on-node triple already rides
 * node.value's cache line, which no side structure can beat. The build is
 * archived (engines/rejected/1_8_0-sidestack.js) with its probe.
 *
 * ALSO MEASURED (not a 1.8.0 property): the recursive-pull depth plateau is
 * V8-scope-shape-sensitive -- adding ANY context slots to createRegistry
 * (four dummy consts suffice) drops the cold-pull overflow point from ~4096
 * to ~2559 on Node 22 by parking the probe on interpreted-size frames.
 * Ceiling scales linearly with --stack-size on every build. Public claims
 * quoting "~4k depth" should say "plateau-dependent, 2.5k-4k, tunable".
 *
...
// 1.9.1: explicit-resource-management support (`using` / Symbol.dispose).
// The well-known symbol exists on Node 20+; the `using` SYNTAX lands later
// (Node 24+ / newer browsers), but stamping the symbol now means every
// handle already works with both `using` and a manual [Symbol.dispose]()
// call. Existence-guarded so environments without the symbol get identical
// object shapes minus the one key. DELIBERATELY NOT stamped on callable
// signal()/computed() handles: those are the creation-cost battleground
// (one extra own-prop write per primitive), and auto-disposing VALUE
// handles at block exit is a footgun besides. Lifecycle objects only:
// the registry (dispose == destroy), effect disposers, createScope
// disposers, and the box PROTOTYPES (zero per-instance cost -- boxes get
// `using` for free through dispose(this) via the shared method).
const SYMBOL_DISPOSE = (typeof Symbol.dispose === "symbol") ? Symbol.dispose : null;
// 1.9.0: set by markDownstream when a write reaches an effect that is
// CURRENTLY RUNNING through its own dep edges (the case the enqueue gate
// must skip). executeEffect's post-body fixed point consumes and clears it.
// Precision for free: unrelated writes never mark the effect, so effects
// that write untracked signals pay nothing at all.
const FLAG_SELF_DIRTY = 1 << 6;
...
export function createRegistry(config) {
    ...
    // 1.9.0: link-churn counters -- topology-stability detector v1 (Reflex
    // graph-reduction study). In a topology-stable steady state the cursor
    // fast-path never touches allocateLink or freeLink at all, so ANY nonzero
    // delta on these counters over an interval == topology churn, and the
    // magnitude == churn volume. Two SMI bumps on cold paths only (cursor
    // misses / severs / disposals); the hot cursor-hit read path is untouched.
    // Divide by op-5 recompute counts from the mutation hook to get a
    // churn-per-recompute stability ratio -- the gating datum for any future
    // static-plan (pattern E) work.
    let statTotalLinkAllocations = 0;
    let statTotalLinkDisposals = 0;
    ...
    // 1.9.0: batch depth at the start of the CURRENTLY RUNNING effect body.
    // markDownstream suppresses FLAG_SELF_DIRTY for marks landing at a depth
    // GREATER than this -- self-writes inside a batch the body itself opened
    // coalesce atomically into the current run instead of iterating. See the
    // fixed-point notes in executeEffect and the header (upstream #235).
    let selfDirtyBaseDepth = 0;
    ...
    // 1.9.0: settled hook -- fires after a flush drains the queue to empty at
    // batchDepth 0 with no buffered errors (quiescence notification for render
    // schedulers / lite-room frame alignment / lite-charts). Same single-
    // nullable-listener discipline as the mutation hook: zero cost detached.
    // The listener runs untracked with no observer; writes inside it trigger
    // a fresh flush (and a fresh settled) -- a listener that always writes is
    // a user-level livelock, the same class as an effect that always writes.
    let settledHook = null;
    function onSettled(fn) {
        if (fn !== null && typeof fn !== "function") throw new TypeError("onSettled: listener must be a function or null");
        const prev = settledHook;
        settledHook = fn;
        return () => { if (settledHook === fn) settledHook = prev; };
    }
    ...
    function allocateLink(source, target) {
        ...
        statTotalLinkAllocations++;   // 1.9.0: cold path -- cursor hits never reach here
    ...
    function freeLink(link, target, source) {
        ...
        freeLinkHead = link;
        activeLinks = (activeLinks - 1) | 0;
        statTotalLinkDisposals++;   // 1.9.0: cold path (sever / divergence / dispose)
    }
    ...
    function disposeNode(node) {
        ...
        while (sLink !== null) {
            ...
            freeLinkHead = sLink;
            activeLinks = (activeLinks - 1) | 0;
            statTotalLinkDisposals++;   // 1.9.0: inlined free path counts too
            ...
            while (link !== null) {
                ...
                if (t.markEpoch !== gv) {
                    ...
                    if ((flags & FLAG_EFFECT) !== 0) {
                        ...
                        } else if ((flags & FLAG_COMPUTING) !== 0 && batchDepth <= selfDirtyBaseDepth) {
                            // 1.9.0: the running effect's own dep moved --
                            // record it for the post-body fixed point.
                            // Branch lives on the SKIP path only (already
                            // queued/computing), off the enqueue hot path.
                            // The depth gate is the two-tier rule: marks
                            // landing INSIDE a batch the body opened
                            // (depth > base) never self-dirty -- an explicit
                            // batch around your own writes is an atomic
                            // update (upstream #235; see header).
                            t.flags = flags | FLAG_SELF_DIRTY;
                        }
                    } else {
    ...
    function flushEffects() {
        ...
        // 1.9.0: settled notification -- clean drain only (error paths threw
        // above), at least one pass ran (a no-op flush is not a settle), and
        // we are at batch depth 0 with an empty queue. Untracked, no observer.
        if (settledHook !== null && passes > 0 && batchDepth === 0 && activeQueueLen === 0) {
            const po = currentObserver, pt = isTrackingDeps;
            currentObserver = null; isTrackingDeps = false;
            try { settledHook(); } finally { currentObserver = po; isTrackingDeps = pt; }
        }
    }
    ...
    function executeEffect(node) {
        ...
        const prevTracking = isTrackingDeps;
        const prevSelfDirtyBase = selfDirtyBaseDepth;

        currentObserver = node;
        currentOwner = node;
        activeObserverCurrentDep = node.headDep;
        isTrackingDeps = true;
        selfDirtyBaseDepth = batchDepth;   // the body's atomicity baseline
        ...
        // 1.9.0 rev2: pre-body clock snapshot -- on a refire, evalVersion is
        // parked here so the ordinary pre-walk re-detects the staleness.
        const savedGV = globalVersion | 0;
        ...
        try {
            ...
            // re-run and on dispose, appended with the exact onCleanup append
            // semantics (single fn -> pair array -> push), so the two
            // registration styles compose in call order. Non-function returns
            // are ignored; a self-disposed body (gen advanced) registers
            // nothing -- the slot may already host a new resident. One typeof
            // per effect run, entirely off the mark/pull path.
            ...
            if (node.gen === savedGen) {
                ...
                // 1.9.0 rev2: inner-write fixed point (adapted from
                // Reflex's invalidateSlow discipline; closes the known
                // effect-self-write conformance gap). markDownstream cannot
                // enqueue a COMPUTING effect, so before 1.9.0 an effect's
                // write to its OWN dependency was silently absorbed by the
                // post-body `evalVersion = globalVersion`. rev2 lets the
                // MARK PHASE do the detection: when the gate above skips a
                // COMPUTING effect it now stamps FLAG_SELF_DIRTY, so the
                // post-body cost here is ONE flag test -- rev1's post-body
                // dep walk (+3-6% on effects writing untracked signals,
                // measured cold-process) is gone, and unrelated writes never
                // mark the effect at all. On a self-mark: evalVersion parks
                // at the pre-body clock and the effect self-enqueues; the
                // ordinary pre-walk drives the iteration, and equality-
                // blocked writes terminate it (no clock advance, no mark).
                // A dep written BEFORE it was read in the same body costs
                // one bounded extra re-run that equality-blocks immediately.
                // True livelocks hit maxFlushPasses, as before. Throwing
                // bodies keep pre-1.9 absorption semantics (flag cleared).
                if (bodyCompleted && (node.flags & FLAG_SELF_DIRTY) !== 0 && (node.flags & FLAG_EFFECT) !== 0) {
                    node.flags &= ~FLAG_SELF_DIRTY;
                    node.evalVersion = savedGV;
                    if ((node.flags & FLAG_QUEUED) === 0) {
                        node.flags |= FLAG_QUEUED;
                        activeQueue[activeQueueLen++] = node;
                        if (mutationHook !== null) mutationHook(7, node.id, 0);
                        refire = true;
                    }
                } else {
                    node.flags &= ~FLAG_SELF_DIRTY;
            ...
            isTrackingDeps = prevTracking;
            selfDirtyBaseDepth = prevSelfDirtyBase;
            // Outside a flush (creation path, scheduler thunks), eager mode
            // must drain the self-enqueue now or the iteration would stall
            // until the next unrelated write. Inside a flush the loop drains
            // it; in sab/manual it waits for batch exit / flush(), matching
            // those modes' delivery contracts.
            if (refire && FLUSH_ON_IDLE_WRITE && !isFlushing && batchDepth === 0) flushEffects();
        }
    }
    ...
    function computed(fn, opts) {
        // 1.10.0: bodies that declare a parameter (or pass {initial}) receive
        // their PREVIOUS value -- Solid createMemo(fn(prev)) ergonomics for
        // scans/reductions. COLD-PATH BY CONSTRUCTION: the decision happens
        // once here, as a wrapper closure created at creation time (creation
        // already allocates closures); pullComputed's call site stays the
        // byte-identical zero-argument `node.computeFn()`. node.value at
        // call time IS the previous value (assignment happens after the
        // body returns), seeded by opts.initial for the first run.
        const wantsPrev = fn.length > 0 || (opts !== undefined && opts.initial !== undefined);
        const node = createNode((opts !== undefined) ? opts.initial : undefined, FLAG_COMPUTED);
        if (wantsPrev) {
            const userFn = fn;
            node.computeFn = () => userFn(node.value);
        } else {
            node.computeFn = fn;
        }
        node.equals = (opts !== undefined && opts.equals !== undefined) ? opts.equals : OBJECT_IS;
    ...
    // 1.9.1: `using b = r.signalBox(0)` disposes the box's node at block
    // exit. Lives on the shared prototypes -- ONE method per registry, zero
    // per-instance creation cost (the 1.5.0 allocation-light contract is
    // untouched). dispose(this) resolves through NODE_PTR with the standard
    // gen guard, so a stale/recycled box disposes nothing.
    if (SYMBOL_DISPOSE !== null) {
        SIGNAL_BOX_PROTO[SYMBOL_DISPOSE] = function () { dispose(this); };
        COMPUTED_BOX_PROTO[SYMBOL_DISPOSE] = function () { dispose(this); };
    }
    ...
    function computedBox(fn, opts) {
        const wantsPrev = fn.length > 0 || (opts !== undefined && opts.initial !== undefined);   // 1.10.0: fn(prev), creation-time wrapper
        const node = createNode((opts !== undefined) ? opts.initial : undefined, FLAG_COMPUTED);
        if (wantsPrev) {
            const userFn = fn;
            node.computeFn = () => userFn(node.value);
    ...
    function effect(fn, opts) {
        ...
        // 1.9.1: `using stop = effect(...)` disposes at block exit. The
        // disposer IS the dispose method (it ignores `this`); idempotent
    ...
    function batch(fn) {
        ...
        try {
            ...
            batchDepth = (batchDepth - 1) | 0;
            if (FLUSH_ON_BATCH_EXIT && batchDepth === 0) flushEffects();
        }
    }
     ...
     */
    /**
     * 1.10.0: handle of the CURRENT owner (the effect/computed whose scope
     * would adopt nodes created right now), or undefined at the root. Pair
     * with runWithOwner to continue work in a scope after an async gap:
     *
     *   const owner = getOwner();
     *   await something();
     *   runWithOwner(owner, () => effect(...));   // adopted, cascades on dispose
     *
     * Gen-guarded like every handle: a disposed owner degrades runWithOwner
     * to rooted execution instead of touching a recycled slot.
     */
    function getOwner() {
    ...
    /**
     * 1.10.0: run `fn` with ownership attached to `ownerHandle`'s node
     * (Solid's runWithOwner on the existing owner/observer split -- the
     * header's long-standing "future work" item). Tracking is OFF for the
     * duration, exactly like createRoot: reads in `fn`'s direct body link
     * nothing; inner effect/computed bodies establish their own scopes and
     * are adopted by the chosen owner. Null/undefined/stale handles run
     * `fn` rooted. Cold path: no hot function is touched.
     */
    ...
    function createScope(fn) {
        ...
        if (SYMBOL_DISPOSE !== null) dispose[SYMBOL_DISPOSE] = dispose;   // 1.9.1: `using` support
        return result;
    }
    ...
    function stats() {
        return {
            ...
            // 1.9.0: link-churn counters (topology-stability detector). Zero
            // deltas over an interval == topology-stable steady state.
            totalLinkAllocations: statTotalLinkAllocations,
            totalLinkDisposals: statTotalLinkDisposals,
    ...
    function destroy() {
        ...
        statPoolGrowths = 0;
        statFlushPasses = 0;
        statTotalLinkAllocations = 0;
        statTotalLinkDisposals = 0;
    ...
    const registry = {signal, computed, effect, signalBox, computedBox, dispose, batch, flush, untrack, createRoot, createScope, getOwner, runWithOwner, onCleanup, stats, destroy, isTracking, hasObservers, observeObservers, forEachObserver, forEachSource, forEachOwned, ownerOf, nodeId, describe, onGraphMutation, onSettled};
    // 1.9.1: `using r = createRegistry()` destroys the whole registry at
    // block exit (every issued handle becomes a safe no-op via the gen bump,
    // exactly as a manual destroy()).
    if (SYMBOL_DISPOSE !== null) registry[SYMBOL_DISPOSE] = destroy;
    return registry;
}
...
export function onSettled(fn) {
    return defaultRegistry.onSettled(fn);
}
