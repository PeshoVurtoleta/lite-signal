...
class ReactiveNode {
    constructor() {
        ...
        this.nextFree = null;
        // 1.3: Intrusive mark-stack pointer. markDownstream chains visited nodes
        // through this field instead of an external array, eliminating the
...
export function createRegistry(config) {
    ...
    function disposeNode(node) {
        ...
        node.nextMark = null;   // 1.3: defensive — disposal during a sweep shouldn't happen, but ensures clean state
     ...
     * Iterative DFS via an intrusive linked-list stack (nextMark) to avoid call-stack growth.
    ...
    function markDownstream(startNode) {
        // 1.3: Intrusive linked-list stack. nextMark on each node chains the
        // visited set together; we walk the chain by popping markHead and
        // following its nextMark. Eliminates markStack[] array bounds checks
        // and consolidates touched memory to the node's own cache line
        // (we already loaded `t` to check t.markEpoch — writing t.nextMark
        // is in the same cache line).
        //
        // gv cached locally: makes the invariant explicit (markDownstream
        // never mutates globalVersion) and lets the JIT keep it in a register
        // across the inner loop.
        const gv = globalVersion;
