...
export function createRegistry(config) {
    ...
    function allocateLink(source, target) {
        ...
        if (freeLinkHead === null) {
            ...
            // Incremental growth (P0): construct exactly ONE link on a free-list
            // miss instead of doubling with an eager construction burst. The
            // capacity LEDGER still doubles (stats() / ceiling semantics are
            // unchanged) -- only the physical allocation is amortized to O(1)
            // per creation. Eliminates multi-ms pauses inside hot loops.
    ...
    function createNode(value, flags) {
        ...
        if (freeNodeHead === null) {
            ...
            // Incremental growth (P0): one node per miss; ledger doubles for
            // stats() continuity. The effect queues and markStack are no longer
            // length-extended here: `arr.length = n` converts a PACKED array to
            // HOLEY permanently (a hidden flush-path tax); sequential
            // `arr[len++] = x` appends keep them packed and auto-grow.
