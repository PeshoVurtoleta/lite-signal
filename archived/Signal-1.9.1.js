/**
 * @zakkster/lite-signal v1.9.1 (candidate)
 ...
 *     (zero per-instance creation cost; the 1.5.0 allocation-light
 *     contract is untouched)
 * Callable signal()/computed() handles are deliberately NOT stamped: one
 * extra own-prop write per primitive is real creation cost on the measured
 * alien-signals battleground, and auto-disposing value handles at block
 * exit is a footgun. Boxes get it free via the prototype.
 *
 ...
 * upstream-conformance gaps (174/177). Mechanism adapted from Reflex's
 * invalidateSlow confirmed-prefix discipline, translated to lite-signal's
 * clock: snapshot globalVersion before the body; after a completed body that
 * performed writes, one dep walk decides whether an OWN dep moved past the
 * pre-body clock -- if so, evalVersion stays at that clock and the effect
 * self-enqueues; the ordinary pre-walk drives the iteration and equality-
 * blocked writes terminate it. Cost when the body writes nothing (the
 * overwhelming case): ONE SMI compare. Effects writing only untracked
 * signals: one cold dep walk, then absorption exactly as before. True
 * livelocks hit maxFlushPasses. Delivery of the fixed-point passes follows
 * the flushStrategy contract (eager: immediate; sab: batch exit; manual:
 * explicit flush()).
 *
...
export function createRegistry(config) {
            ...
            while (link !== null) {
                ...
                if (t.markEpoch !== gv) {
                    ...
                    if ((flags & FLAG_EFFECT) !== 0) {
                        ...
                        } else if ((flags & FLAG_COMPUTING) !== 0) {
    ...
    const registry = {signal, computed, effect, signalBox, computedBox, dispose, batch, flush, untrack, createRoot, createScope, onCleanup, stats, destroy, isTracking, hasObservers, observeObservers, forEachObserver, forEachSource, forEachOwned, ownerOf, nodeId, describe, onGraphMutation, onSettled};
