 ...
 *     an O(1) source.tailSub dedup + cursor splice (severTail still reclaims the
 *     displaced links). Dependency re-tracking is now O(N) regardless of read
 *     order, not O(N^2). Wide reordering nodes: ~48x locally (600-dep flip:
 *     2810ms -> 58ms). First-track of a wide node: ~4.4x (4000-dep: 48ms -> 11ms).
 *
...
export function createRegistry(config = {}) {
    ...
    function allocateLink(source, target) {
        ...
        // O(1) same-pass dedup: if we already linked this source to this target
        // during THIS tracking pass, its sub-list tail still points at us. Covers
        // reading the same source twice (the only thing the old O(N) scan could
        // find), without the linear walk that made dynamic re-tracking O(N^2).
        const lastSub = source.tailSub;
        if (lastSub !== null && lastSub.target === target) {
            // Reposition the existing link to the cursor so read order stays stable.
            if (lastSub !== expected) {
                let p = lastSub.prevDep, n = lastSub.nextDep;
                if (p !== null) p.nextDep = n; else target.headDep = n;
                if (n !== null) n.prevDep = p; else target.tailDep = p;
                lastSub.nextDep = expected;
                if (expected !== null) {
                    let pe = expected.prevDep;
                    lastSub.prevDep = pe;
                    expected.prevDep = lastSub;
                    if (pe !== null) pe.nextDep = lastSub; else target.headDep = lastSub;
                } else {
                    let tail = target.tailDep;
                    lastSub.prevDep = tail;
                    if (tail !== null) tail.nextDep = lastSub; else target.headDep = lastSub;
                    target.tailDep = lastSub;
                }
            } else {
                activeObserverCurrentDep = expected.nextDep;
            }
