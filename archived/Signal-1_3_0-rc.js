...
export function createRegistry(config) {
    ...
    // set/update/subscribe are method-invoked (s.set(v), s.update(fn),
    // s.subscribe(fn)), so `this` is the read function and this[NODE_PTR] is the
    // node. peek() stays a closure: its body is too cheap to absorb the node
    // recovery, so a closure beats the property load there.
    function sharedSet(value) {
