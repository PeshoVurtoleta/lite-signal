...
`torture/` holds eighteen scenarios in two groups, a shared `helpers/` module and a
runner. **None are benchmarks.**

```bash
npm run torture              # everything
npm run torture:semantic     # correctness only — fast, deterministic, CI
npm run torture:soak         # resource soaks — nightly / pre-publish

node bench/torture/run.mjs --list
node bench/torture/run.mjs oracle glitch      # substring match
node bench/torture/run.mjs -g soak -s 30 -b   # 30s soaks, bail on first failure
```

The runner spawns each scenario as a child process rather than importing it.
That is deliberate: several scenarios assert on global pool accounting and on
the default registry, so running two in one process would let the first one's
residue poison the second's baseline. Each file also stays directly executable
for when you are iterating on one.

### Resource soaks (`graph-fuzzer`, `scheduler-bench`, `torture-soak`)
...
# Semantic torture (fast, deterministic, CI-friendly)
node --expose-gc bench/torture/oracle-fuzzer.mjs
node --expose-gc bench/torture/glitch-hunter.mjs
node --expose-gc bench/torture/work-accounting.mjs
...
### Semantic torture (`oracle-fuzzer`, `glitch-hunter`, `work-accounting`)

The soaks above share a single oracle: nothing threw, and the pool came back to
baseline. Both are liveness questions. Neither reads a value and asks whether it
is *right* — so an engine that returns stale values from every computed passes
all three green.

That is not hypothetical. Flipping the clean short-circuit in `pullComputed`
from `<= 0` to `<= 1` — a one-character off-by-one of exactly the kind a perf
tweak makes — throws nothing, keeps the pool perfectly balanced, and is
invisible to all three soaks. The unit suite catches it, but only on small
hand-written graphs; the soaks are the only thing exploring large random
topologies, which is precisely where a versioned push-pull engine with
short-circuits is most likely to be wrong.

These three close that gap, each on a different axis:
...
| `cleanup-return-torture.mjs` | Does an effect's returned cleanup honour timing, compose order, and the self-dispose guard? | The 1.8.0 cleanup-return path: a returned cleanup runs before re-run and on dispose, composes AFTER imperative `onCleanup(fn)` in forward call order, is suppressed by the self-dispose gen-guard, and leaves computeds' function-values untouched. Flagship is a differential — `return fn` must be observationally identical to `onCleanup(fn)` as the last statement — across 400 seeds. Skips on pre-1.8.0. Also pins the 1.8.0 disposal-crash fix by MECHANISM: five named cursor-geometry variants (dispose a linked-unread computed; dispose multiple linked-unread sources; the sharpest case — read s1 then dispose s2 with the cursor parked exactly on the freed link; that case plus a returned cleanup; and a survivors-intact check that headDep was not wiped), so a regression reports which cursor position broke rather than a random fuzz seed. |
...
| `flush-torture.mjs` | Do the flush strategies converge, is only *delivery* deferred (never the value), and does `.subscribe()` honour its contract? | The three `flushStrategy` modes and `r.flush()` via a cross-strategy differential (identical settled values once drained). Plus: the load-bearing invariant that a computed **pull is fresh while effects are still deferred** (only delivery defers, never the value); FLAG_QUEUED enqueue-dedup counted via op-7 (50 deferred writes enqueue once, not 50 — run-count alone is masked by execute-time dedup); invalid `flushStrategy` token throws; re-entrant `flush()` (isFlushing guard); explicit flush inside a batch drains mid-batch; sab nested-batch flushes only at the outermost exit; and the full `.subscribe()` surface under each strategy. Skips on pre-1.7.0. |
...
| `owner-torture.mjs` | Do `getOwner`/`runWithOwner` adopt live owners, degrade stale handles to rooted, and isolate deps? | Owner-handle capture/restore: live-owner adoption (an adopted effect cascade-disposes with its owner), the ABA degradation (a captured owner disposed + slot recycled must degrade `runWithOwner` to ROOTED, never adopt into the recycled resident), dep isolation (reads inside `runWithOwner` form no edge into the enclosing observer), `runWithOwner(undefined)` rooting, nested restore, and a 300-seed capture/dispose/recycle/restore fuzz. Adoption is the reachable/catchable core; the ABA guard + dep isolation are over-determined defense-in-depth (documented). Runs 1.6.0+. |
...
**Mutation evidence.** Five real defects were injected into `Signal.js` and every
torture file was run against each:

| Injected defect | soaks (3) | oracle | glitch | work |
| --- | --- | --- | --- | --- |
| `pullComputed` short-circuit off-by-one (stale read) | blind | **caught** | **caught** | **caught** |
| version stamp skipped 1-in-16 (dropped update) | blind | **caught** | blind | blind |
| default equality `==` instead of `Object.is` | blind | **caught** | blind | blind |
| default equality `===` instead of `Object.is` (`-0`/`NaN`) | blind | **caught** | blind | blind |
| abandoned dependencies never unsubscribed | **caught** (2 of 3) | blind | **caught** | **caught** |
...
mutant leaks links, which is exactly what their pool oracle is for), and the
semantic files own meaning. Keep both.

Two further candidate defects turned out to be **inert**, and are recorded here
so nobody re-derives them: aliasing `effectQueueB` to `effectQueueA` does not
change observable cascade ordering, and disabling the eval memo in `pullComputed`: the `markEpoch` short-circuit immediately below absorbs it,
so no redundant work occurs. Worth knowing that the two short-circuits are
genuinely defence-in-depth rather than one guarding the other.


### What is deliberately NOT here

`scheduler-storm.mjs` does not test priority inversion or starvation, and that
is not an omission. `scheduler` is a user-supplied callback: the engine hands it
a `run` thunk and the scheduler must eventually invoke it. Signal.js has no
priority concept anywhere, so there is nothing to invert, and a scheduler that
never calls `run` is honouring its contract rather than starving. A test for
either would be testing the harness, not the engine — so the file pins the
engine-side hazards instead (gen-bound thunks, ABA across slot recycle,
coalescing while deferred) and the "never invokes `run`" case asserts only that
it does not wedge the flush or starve unrelated effects.

A dynamic-churn scenario was considered and rejected as largely redundant:
`oracle-fuzzer` already generates `select` nodes with steered selectors, and
