 ...
 *
 * 1.7.0 NOTE: this scenario is createScope-based and SKIPPED on 1.4.4 (no
 * createScope). 1.7.0 ships createScope + getOwner + runWithOwner, so it RUNS
 * NATIVELY here -- no SKIP. Every scenario (1-7) was confirmed against 1.7.0's
 * real createScope engine and passed as written: live adoption cascades on
 * owner dispose, stale handles degrade to rooted, deps stay isolated. No
 * assertion was speculative; nothing needed pinning to a divergent behaviour.
 *
