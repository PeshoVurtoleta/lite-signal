...
/* -- 9. grow mode still has a ceiling: the 16x link limit ------------------- */
{
    ...
    // against 1.7.0 Signal.js:248 `const maxLinkLimit = currentLinkCapacity * 16;`
    // and :407 `throw new CapacityError("links", maxLinkLimit);`). At the ceiling
    ...
    // and not one chunk over. Observed on 1.7.0: multiplier 16, so maxLinks 8 ->
