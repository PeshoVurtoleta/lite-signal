...
## [1.6.0-rc.2] -- 2026-09-20

Verification + docs increment over `1.6.0-rc.1`; **no engine change past the
version banner comment** (`Signal.js` byte-identical, banner line only). Ports
the standing perf-gate lane from the canonical 1.5.2 line onto this engine.
...
- Two `mustFail` controls that MUST trip the gate: a per-op `{x,y,z,w}` object
  allocator, and the documented `signal()`+dispose churn (264 B/op on this
  engine, per `[1.6.0-rc.1]`). lite-perf-gate DEFAULT thresholds, untouched
  (maxScavenges 2, maxRetainedKB 64, maxOldGen 0, maxArrayBuffersKB 64).
