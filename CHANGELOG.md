...
## [1.8.0-beta.1] -- 2026-09-20

Verification + docs increment over `1.8.0-beta`; **no engine change past the
...
- `test/36-perf-gate.test.mjs` + `npm run test:gate`
  (`node --expose-gc --max-semi-space-size=4 --test test/36-perf-gate.test.mjs`),
...
the 1.8.0-alpha.6 engine. Supersedes 1.8.0-alpha.6. Only `createRegistry`'s *cold*
...
## [1.8.0-alpha.5] -- 2026-08-18

Brings the `bench/torture/` suite and the throwing-`equals` unit test to **full
parity with the shipped 1.4.4 canon**. This is a **verification-only** cut: the
engine (`Signal.js` + `Signal.d.ts`) is **UNCHANGED, byte-identical** to
`alpha.4` -- every ported scenario was run against the 1.8.0 engine and pins its
**actual** behaviour; no assertion was softened and no budget widened.

### Added -- torture parity (17 -> 22 scenarios)

- Registered the already-present-and-passing `owner-torture` (`getOwner` /
  `runWithOwner` capture-restore + ABA degradation, a 1.6.0 primitive).
- Ported four scenarios that all run **natively** on the 1.8.0 engine:
  ...
  `AggregateError`, buffer drain over 4096 cycles), `deep-chain-torture`
  (`pullComputed` recursion fail-closed with a `RangeError` vs the heap-iterative
  push path), and `zerogc-torture` (the falsifiable zero-GC gate via
  `measureAllocs` / `measureOps` + `stats()` counters, with the `ZEROGC_BREAK`
  self-test). None needed pinning to a divergent behaviour.
- `capacity-torture` gained section 9: the **16x grow-link ceiling** (multiplier
  confirmed against `Signal.js:285`), fail-closed with a `links` `CapacityError`.
- The three soaks (`graph-fuzzer`, `scheduler-bench`, `torture-soak`) gained an
  **Int32Array value-oracle** (a fixed shadow window over the writable signals,
  allocated once, zero per-tick allocation, full sweep at teardown).
- New `test/30-throwing-equals.test.mjs` pins all **five** `equals()` invocation
  sites -- 3 callable (signal set pre-check, batch revert, computed re-eval) plus
  the 2 `signalBox` `boxSet` sites (pre-check, revert).

The `cleanup-return`, `owner`, `flush`, `scope` and `createRoot`-based scenarios
all run natively on this engine. On the 1.8.0 engine the runner now executes
**18 semantic** scenarios and cleanly SKIPS the one remaining later-version
scenario (`dispose-torture` 1.9.0); the suite total is **22 (19 semantic + 3
soak)**.

### Verified -- full suite green on the 1.8.0 engine (Node 22, `--expose-gc`)

- **Torture:** 22/22 (`torture:semantic` 19/19 -- 18 executed, 1 clean skip
  `dispose` 1.9.0; `torture:soak` 3/3, value-oracle matched).
- **Unit (plain `npm test`):** 499 tests, 498 pass, 0 fail, 1 skip (the
  architectural SSR N/A in `17-reactivity`).
- **Unit (`npm run test:gc`):** 507 tests, 506 pass, 0 fail, 1 skip.

## [1.8.0-alpha.4] -- 2026-07-27

Rolls up the post-`1.8.0-preview` changes into the `alpha.4` tag. The
effect-cleanup-return feature (an effect body may `return` a cleanup) ships in
the preview and is unchanged here; this cut carries the dangling-cursor crash fix
(below, now also exercised against the cleanup-return path), 100% coverage on
`Signal.js`, and the completed torture surface for the cleanup-return feature.

### Added -- `cleanup-return-torture` now executes on the 1.8.0 engine

`bench/torture/`'s `cleanup-return-torture` scenario -- present but
feature-skipped on the 1.5.x-1.7.x lines -- now **runs** (400 seeds), since an
effect body returning a cleanup is a 1.8.0 primitive. It pins the timing (the
returned cleanup runs before each re-run and once on dispose, same points as
imperative `onCleanup`), the **compose order** (a returned cleanup is appended
after any `onCleanup` registered in the same body, so both fire in call order),
the self-dispose guard (a body that disposes itself registers nothing), and the
computed exclusion (a computed returning a function keeps it as the value, never
a cleanup). On the 1.8.0 engine the runner therefore executes **13 semantic**
scenarios and cleanly SKIPS the one remaining later-version scenario
(`dispose-torture` 1.9.0); the suite total is unchanged at **17 (14 semantic +
3 soak)**.

### Verified -- full suite green on the 1.8.0 engine (Node 22, `--expose-gc`)

- **Torture:** 17/17 -- `torture:semantic` 14/14 (13 executed incl.
  `cleanup-return-torture`, 1 clean skip: `dispose` 1.9.0), `torture:soak` 3/3
  (zero errors, every pool back to its leaf-only floor).
- **Unit (plain run):** 492 tests, 472 pass, 0 fail, 1 skip (the architectural
  ...
  `@zakkster/lite-devtools`; with that sibling resolvable the suite is 491 pass +
  1 skip. Engine-only, excluding `test/25`: 473 tests, 472 pass, 1 skip. (The c8
...
from the plain public API, and it composes with the new cleanup-return path (the
effect's returned cleanup still runs). Fixed with a one-line cursor repair in
`disposeNode` (advance the cursor to the next surviving dep before the free);
O(1), disposal path only, no steady-state cost. The now-dead `freeLink` `-1`
defensive ternaries were removed with it (replaced by the always-live `source.id`
/ `target.id`). Pinned by a regression test to add to `test/12-coverage.test.mjs`.

**Coverage:** the full box/owner/opcode closure suite (ported forward from 1.7.0)
plus this fix and a test for the new cleanup-return array-append arm bring
`Signal.js` to **100% statements / branches / functions / lines** (484 tests,
483 pass, 1 skip). The two provably-unreachable clamps (link-ledger,
`batchEpoch` 2^32 wraparound) carry `/* c8 ignore */` with proofs. See
`COVERAGE-NOTES.md`; reconfirm under codify.

**Hot-path parity note:** this touches `disposeNode` and `freeLink`; re-baseline the
VersionMatrix engine sha256 on the next publish. `executeEffect` also changed on
this line for cleanup-return, independently.

### Changed -- test file numbering

`test/28-scope.test.mjs` renamed to `test/29-scope.test.mjs` to resolve the
duplicate `28-` prefix it shared with `28-run-with-owner.test.mjs` (carried over
from the 1.7.0 fix; `createScope` is the 1.6.0 primitive, sequenced after
`runWithOwner`).
...
reproduced by `harness/owner-hazard-repro.mjs`:
...
`harness/owner-hazard-repro.mjs ./Signal.js` reports `VERDICT: SAFE` on
...
- **`harness/owner-hazard-repro.mjs ./Signal.js`** reports `VERDICT:
...
### Verified -- fresh 1.8.0 bench sweep (Apple M4 Pro / Node 26)

Both v3 instruments were re-run in full on the 1.8.0 engine on **Apple
M4 Pro darwin/arm64, Node 26.3.1**, with `#STAMP`-verified outputs
committed to `bench/r.txt` (microscope aggregate, 4 engines across the
six first-party shapes) and `bench/rb.txt` (mirror sweep, Andrii's
canonical adapter verbatim, lite vs alien across 47 rows,
isolated-per-row, 10 reps). Fresh Andrii 15-framework log at
`bench/AndriiVolynetsReactiveBench.log`.

- **Andrii Volynets js-reactivity-benchmark position moves from 4th to
  3rd of 15** with geomean **95.4 ms**; lite is now ahead of 4th-place
  @reactively (136.0, lite 1.43x faster) and 5th-place Preact Signals
  (148.8, ~56% ahead), behind only alien-signals (77.5) and reflex
  (89.7). **Outright-fastest-of-15 wins jumped to 20 shapes** (was 3 on
  1.3.0 log, 4 on 1.7.0-preview): the eight canonical propagation shapes
  (`avoidablePropagation`, `broadPropagation`, `deepPropagation`,
  `diamond`, `mux`, `repeatedObservers`, `triangle`, `unstable`), all four
  wide fan-in / fan-out shapes (`manyEffectsFromOneSource`,
  `manySourcesIntoOneComputedEffect`,
  `manySourcesIntoOneComputedEffectWithDirect`,
  `manySourcesIntoOneDirectEffect`), `molBench`, and seven of the
  rectangular / layered DAG shapes (`64x6`, `24x8`, `120x7`, `40x10`,
  both `256->512x32 layered-dag` variants, and `10x10 dynamic component`).
  Top-3 count: **29/47** (was 21 on 1.3.0). The dramatic wins jump is
  driven by the same engine bytes across every propagation function
  except `executeEffect`, which is where the 1.8.0 effect-cleanup-return
  check landed.
- **Microscope aggregate on M4 Pro (r.txt):** on the six v3 microscope
  shapes, lite wins vs alien on **MUX +34.3%** (fan-in), **SELECTIVE DAG
  +19.3%**, **DYNAMIC DAG +16.8%**, and **BROADCAST +9.4%** (recovered
  from +6.2% on 1.7.0-preview) -- the allocation-heavy dynamic shapes and
  fan-in / fan-out. The wins hold within noise of the 1.7.0-preview M4 Pro
  sweep despite the `executeEffect` change (one `typeof` per effect run,
  off the mark/pull path). Alien ahead on **KAIROS -15.9%** (widened
  slightly from -11.7% on 1.7.0-preview) and **DEEP CHAIN -72.6%** (the
  architectural weak spot: recursive JS-stack computed resolution against
  alien's flatter chain). Speed wins vs alien: **4/6**; heap wins:
  **5/6** with the sixth being a shared-zero on BROADCAST.
- **Microscope heap (the actual story) on M4 Pro:** on every shape where
  GC pressure exists at all, lite allocates **one to four orders of
  magnitude less transient heap than alien** -- DEEP CHAIN 0.5 KB vs 1062
  KB (>2000x, on the shape lite loses on time), MUX 0 KB vs 781 KB,
  KAIROS 23 KB vs 802 KB (34.3x), DYNAMIC DAG 3.3 MB vs 59.8 MB (18.3x),
  SELECTIVE DAG 7.7 MB vs 77.8 MB (10.0x). Against preact-signals and
  solid-signals the heap gap is even wider on the fan-in / fan-out
  family; solid allocates ~17 MB on MUX and ~16 MB on SELECTIVE DAG where
  lite allocates 0 KB and 7.7 MB on the same shapes. The differentiated
  position is ALLOCATION, not raw propagation speed -- competitive-to-
  winning throughput with dramatically lower GC pressure, and that is
  the headline that reproduces across hosts.
- **Mirror sweep on M4 Pro (rb.txt):** Andrii's canonical 47-shape suite
  re-run isolated-per-row lite vs alien on M4 Pro reproduces the same
  honest framing: lite runs **parity-to-behind alien on throughput**
  across the head-to-head suite (wins outright on **4/47** --
  `1000x5 - 25 sources (wide dense)` +12.0%,
  `manySourcesIntoOneComputedEffectWithDirect` +31.1%,
  `manySourcesIntoOneComputedEffect` +29.5%, and `molBench` +0.3%), weak
  on the deep/layered-burst family. That head-to-head result and the
  "3rd of 15 with 20 outright wins" both live on the same log and are
  not contradictory: the outright-of-15 rankings look at each shape
  across every framework in the field, so shapes where alien beats lite
  by 20-30% but reflex beats both by another 10% and everyone else is
  slower can still count as a lite outright win when measured that way.
- **Third-party engine versions carried forward** from the 1.7.0-preview
  sweep (alien-signals 3.2.1, @preact/signals-core 1.14.2,
  @vue/reactivity 3.5.35, solid-js 1.9.13). No peer bumps this cycle;
  the deltas here are the 1.8 engine's own footprint (the `executeEffect`
...
### Tooling, types, and docs

- **`Signal.d.ts` now types the cleanup return.** It declared
  `effect(fn: () => void, ...)`, so the headline 1.8.0 feature did not
  typecheck for TypeScript consumers at all -- and `Signal.d.ts` ships in
  `files[]`. Added `EffectCleanup` and `EffectBody` (`() => void |
  EffectCleanup`) and widened both `effect()` signatures (the `Registry`
  method and the top-level export). Verified with `tsc --strict`: returning a
  cleanup typechecks, plain `void` bodies still typecheck (no regression), and
  a computed returning a function still yields the function as its **value**.
- **Interim behavioral coverage for the cleanup return.** The Test-coverage
  note above defers this feature's tests to a later preview, which left the
  flagship 1.8.0 change shipping unpinned. `harness/smoke.mjs` (which
  **asserts**, and runs first in `harness:all` and `npm run verify`) now
  carries T6-T9: the cleanup fires before the next re-run *and* on dispose; it
  composes with `onCleanup` in call order; non-function returns
  (number/string/null/object/undefined) are ignored without throwing; and a
  computed returning a function still treats it as the VALUE. These are a
  regression pin, not a substitute for the full `test/` cases.
- **VersionMatrix manifest fixed and moved.** An updated manifest sat at the
  repo root, where the gate never reads it -- `pre-publish.mjs` spawns
  `matrix.sh` with `cwd: harness/VersionMatrix`, and `matrix.sh` `cd`s to its
  own directory, so `./manifest.json` always resolves inside `VersionMatrix/`.
  The gate was therefore still running against the stale `floor: 1.3.0` /
  `rolling: 1.5.0-alpha.1` baselines. Its intent (move the baselines forward)
  is now applied in the file the gate actually reads -- **but not with the
  labels it proposed**: `floor: "1.5.0"` and `rolling: "1.7.0-eager"` are not
  npm-installable (no plain `1.5.0` was ever published, and `1.7.0-eager` is a
  toe-to-toe column label, not a version). `matrix.sh` `swap`s floor/rolling in
  from the registry, so those would have failed before measuring anything.
  Now: `floor: "1.3.0"` (the current `latest` stable anchor) and
  `rolling: "1.7.0-preview.2"` (the `preview` tag -- this candidate's true
  predecessor), with a `_notes` line recording the npm-installable constraint.
- **The `~5,000 chained computeds` depth claim is retired** from README and
  `llms.txt`, per the "Also measured" finding above. The recursive-pull ceiling
  is a **plateau, not a constant**: 4096 on 1.8.0 at Node 22's default stack
  (overflow at 4097), V8-scope-shape-sensitive, and linear in `--stack-size`
  (measured: `--stack-size=4000` lifts the same engine to 16384). The docs now
  say **plateau-dependent, 2.5k-4k on a default stack, tunable**, and point at
  `npm run profile:pull` to pin it per build.
...
- **Full suite green** against the 1.6.0 engine: 443 tests, 442 pass, 0 fail,
  ...
  `createScope` tests in `test/29-scope_test.mjs`; the 1.5.0 box and createRoot
