...
## [1.11.0-preview.1] -- 2026-09-20

Verification + docs increment over `1.11.0-preview`; **no engine change past the
...
line onto this engine. (Orthogonal to the pending `[1.11.0-alpha.0]` promotion
above -- this increments the published preview line only.)

### Added -- perf-gate lane (`@zakkster/lite-perf-gate`, devDependency only)

- `test/39-perf-gate.test.mjs` + `npm run test:gate`
  (`node --expose-gc --max-semi-space-size=4 --test test/39-perf-gate.test.mjs`),
...
Two cold-path primitives the profiler tools (`lite-trace`, `lite-hud`,
`lite-devtools`) consume directly. No node-shape change; the pinned handle
shapes from `21-perf-pins` are untouched.

### Added
- `signal(v, {name})` / `computed(fn, {name})` / `effect(fn, {name})`. Names
  live in a registry-side `Map<id, name>`, written only on named creation and
  read only by `describeNode()` / `whyDirty()` (both cold). Unnamed nodes pay
  a single guarded comparison on the creation path and carry no name key.
- `describe()` / `describeNode()` gain `name` when present (absent otherwise).
- `whyDirty(handle)` -- pull-side diagnostic. For a computed, walks its deps
  and reports which one's version moved past the observer's `evalVersion`
  (the SAME predicate `pullComputed` uses, `((dep.version - evalVer)|0) > 0`),
  then traces transitively to the root signal write via `rootCause`. Read-only:
  it never pulls, so it cannot perturb the graph. Returns `[]` for a clean
  computed and for non-computed handles.
- `nodeNames` cleared by `destroy()`.

### Unchanged (proven, not asserted)
- Hot propagation bodies `markDownstream` / `flushEffects` / `executeEffect` /
  `pullComputed` are **byte-identical** to 1.9.0 (sha256 over extracted bodies).
- Bars: `1to1batch` neutral (write path is the frozen set); `createSignals` /
  `createEffects` within noise (the name comparison on the unnamed path is free).
...
- **New: `test/33-computed-selfdirty-prev-owner.test.mjs`** -- a single
...
file used `33-computed-selfdirty-prev-owner_test.mjs` (underscore);
renamed to `.test.mjs` (period) in the delivered tree to match the
ecosystem-wide convention (`NN-name.test.mjs`) and pick up the standard
`test/*.test.mjs` glob. `_test.mjs` would be silently ignored by
`node --test`.
...
This is NOT ledger §0b #6 (per-call closure-var load of a primitive). It is a
hoisted function reference resolved once at registry build time and is a NEW
mechanism by the v1 roadmap §0b line 100 definition.
...
version bumping) -- design decisions accepted by §0b and not in scope here.
...
- **Full suite green** against the 1.6.0 engine: 443 tests, 442 pass, 0 fail,
  ...
  `createScope` tests in `test/28-scope_test.mjs`; the 1.5.0 box and createRoot
