/**
 * @zakkster/lite-signal v1.11.0-preview.1
 ...
 *   variant with a drain-complete tail -- the same const-selection discipline as the
 *   1.7.0 flushStrategy closures. A DEFAULT registry binds the verbatim plain drain,
 *   so its flushEffects (and the hot write path that calls it) is byte-identical to
 *   1.10.0's: zero cost BY CONSTRUCTION, not by a per-flush check. onSettled() on a
 *   registry built without the capability throws, pointing at the option.
 *   onSettled(fn) fires ONCE per top-level, non-empty, clean drain (not per effect,
 *   not on re-entrant or empty flushes, not on a flush that threw), and returns an
 *   unsubscribe. This retires the rejected always-checked hook (ledger #17): the
 *   feature returns with its cost inverted, decided before the first closure is built.
 * Hot bodies markDownstream / executeEffect / pullComputed are byte-identical to
 * 1.10.0; flushEffects's PLAIN body is byte-identical (only its binding became a
 * build-time-selected const). See ROADMAP-1_9-REBUILT s1.11.0.
 *
 * Previous 1.10.0 header follows.
 * --------------------
 * @zakkster/lite-signal v1.10.0 (rebuilt line, candidate)
 * Copyright (c) Zahary Shinikchiev <shinikchiev@yahoo.com>
 * MIT License
 * --------------------
 * 1.10.0 -- named nodes + whyDirty(): the tracing foundation. TWO cold-path
 * primitives the profilers consume directly, zero node-shape change.
 *   - signal(v,{name}) / computed(fn,{name}) / effect(fn,{name}): names live
 *     in a registry-side Map<id,name>, populated ONLY when a name is given;
 *     unnamed nodes pay nothing and keep their pinned handle shapes.
 *     describe()/describeNode() gain `name` when present.
 *   - whyDirty(handle): pull-side diagnostic. Walks a computed's deps and
 *     reports which one's version moved past the observer's evalVersion --
 *     the SAME predicate pullComputed uses, ((dep.version - evalVer)|0) > 0 --
 *     then traces transitively to the root signal write. Read-only (it never
 *     pulls, so it cannot perturb state), allocates freely; a user-cold-path
 *     call the engine never invokes.
 * Hot propagation bodies (markDownstream / flushEffects / executeEffect /
 * pullComputed) are BYTE-IDENTICAL to 1.9.0 (sha256-proven); the feature is
 * confined to creation + cold diagnostics. See ROADMAP-1_9-REBUILT s1.10.0.
 *
 * Previous 1.9.0 header follows.
 * --------------------
 * @zakkster/lite-signal v1.9.0 (rebuilt line, candidate)
 * Copyright (c) Zahary Shinikchiev <shinikchiev@yahoo.com>
 * MIT License
 * --------------------
 * 1.9.0 -- `using` / Symbol.dispose on lifecycle objects. SINGLE-FEATURE
 * release on the 1.8.0 base (the rebuilt 1.9 line; the previous 1.9.x/1.10
 * engines were rejected -- see ROADMAP-1_9-REBUILT and ledger #16/#17).
 * Existence-guarded; manual [Symbol.dispose]() works on Node 20+, `using`
 * syntax lights up on Node 24+. Registry -> destroy(); effect/scope
 * disposers -> themselves (idempotent, gen-guarded); boxes via SHARED
 * prototypes (zero per-instance cost). Callable value handles unstamped.
 *
 * Original 1.8.0 header follows.
 * --------------------
 * @zakkster/lite-signal v1.8.0 (original header)
 * Copyright (c) Zahary Shinikchiev <shinikchiev@yahoo.com>
 * MIT License
 * --------------------
...
// 1.9.0 (rebuilt line): explicit-resource-management (`using` / Symbol.dispose).
// Well-known symbol exists Node 20+; `using` syntax lands Node 24+. Stamped,
// existence-guarded, on LIFECYCLE OBJECTS ONLY: registry (dispose==destroy),
// effect disposers (the disposer IS the method), createScope disposers, and
// the box PROTOTYPES (one shared method per registry, zero per-instance
...
export function createRegistry(config) {
    ...
    // never collides with a prior name; stale entries for disposed named nodes
    // are harmless (bounded, debug-scope) and cleared by destroy().
    ...
    // 1.11.0: settled capability, decided ONCE here. Selects a flushEffects variant
    // at build time (see near flushEffects); it is never a per-flush check. A default
    // registry pays nothing and its flushEffects is byte-identical to 1.10.0's.
    const CAP_SETTLED = (config !== undefined && config.settled === true);
     ...
     * @private
     */
    const flushEffectsPlain = function () {
    ...
    // 1.11.0: onSettled -- drain-complete notification, selected at build time.
    //
    // The rejected 1.9.x onSettled was an always-checked drain hook: ~7ns/flush
    // through V8's inline budget, a +27-31% batched-update regression (ledger #17).
    // This inverts the cost model. `flushEffects` below is a const chosen ONCE from
    // { plain, settled }. A default registry (CAP_SETTLED false) binds the verbatim
    // plain drain, so the hot write path calls a byte-identical closure and pays
    ...
    // calls (wasFlushing) and empty flushes (no queued work) do not notify, and a
    // drain that throws propagates before the tail, so "settled" always means the
    // graph actually quiesced.
    const flushEffectsSettled = function () {
        const wasFlushing = isFlushing;
        const hadWork = activeQueueLen > 0;
        flushEffectsPlain();
        if (!wasFlushing && hadWork) fireSettled();
    };
     ...
     * @private
     */
    // 1.12.1 (ledger #18): the cleanup-return APPEND LADDER lives here, OUT OF LINE.
    // 1.8.0 inlined all 15 lines of it into executeEffect -- a hot body -- which pushed that
    // body past V8's inline/code-size budget and cost 11-23% on effect-dense graphs
    // (bisected: reverting ONLY that hunk restored 1.7.0 exactly). Same failure mode as
    // ledger #17: BYTES, not instructions. The hot body now carries a single branch --
    // `if (ret !== undefined)` -- and the common case (an effect returning nothing) is one
    // undefined-compare. This cold function is entered only when a body actually returns
    // something, i.e. it rides a branch that is already cold: the counter-placement rule.
    // Semantics are IDENTICAL to 1.8.0: auto-detected (no API change), conditional returns
    // work on any run, composes with imperative onCleanup in call order, and a self-disposed
    // body (gen advanced) registers nothing -- the slot may already host a new resident.
    function registerCleanupReturn(node, ret, savedGen) {
        if (typeof ret !== "function" || node.gen !== savedGen) return;
    ...
    function executeEffect(node) {
        ...
        try {
            ...
            if (ret !== undefined) registerCleanupReturn(node, ret, savedGen);
        } finally {
    ...
    function signal(initial, opts) {
        ...
        node.version = globalVersion;
        statSignals++;
        if (opts !== undefined && opts.name !== undefined) nodeNames.set(node.id, opts.name);
    ...
    // 1.9.0: `using b = r.signalBox(0)` -- on the SHARED prototypes, one
    // method per registry, zero per-instance creation cost; dispose(this)
    // resolves through NODE_PTR with the standard gen guard.
    ...
    function effect(fn, opts) {
        ...
        // 1.9.0: `using stop = effect(...)` disposes at block exit; idempotent
    ...
    /**
     * Capture the currently-active owner as an opaque, gen-stamped handle
     * (`undefined` if called outside any effect/computed body). Companion to
     * `runWithOwner`: capture the lifecycle scope now, restore it later --
     * typically across an async boundary. Landed in 1.5.0-beta.3 to unblock
     * the profiler tooling; carried forward unchanged on every line since.
     *
     * The handle is a `describeNode` descriptor stamped with the pool slot's
     * generation counter. It is safe to hold across async boundaries: if the
     * captured owner is disposed and its slot is subsequently recycled by an
     * unrelated effect/computed, the gen no longer matches and `runWithOwner`
     * treats the handle as stale (degrades to rooted execution). Same ABA guard
     * used by `describe` / `nodeId` / `forEachOwned` / `ownerOf`.
     *
     ...
     * cascade-dispose when the owner re-runs or is disposed. Landed in
     * 1.5.0-beta.3, carried forward unchanged on every line since.
     *
     * Degradation semantics for handles that cannot be re-attached:
     * - `null` / `undefined` -> rooted execution (createRoot semantics for the
     *   ownership axis; created children survive, caller disposes them).
     * - stale handle (owner disposed, slot recycled) -> rooted execution. Never
     *   adopts into the unrelated new resident of the recycled slot; never
     *   adopts into a corpse (a disposed owner whose slot is not yet recycled).
     * - handle for a live signal (not an effect/computed) -> rooted execution
     *   (signals cannot own).
     *
     * Nulls the tracking observer for the duration of `fn` (the same pairing
     * as `createRoot`): reads directly inside `fn` do NOT link into any
    ...
    function createScope(fn) {
        ...
        if (SYMBOL_DISPOSE !== null) dispose[SYMBOL_DISPOSE] = dispose;   // 1.9.0: `using` support
        return result;
    }
    ...
    function describeNode(node) {
        ...
        const nm = nodeNames.get(node.id);   // 1.10.0: cold; undefined for unnamed nodes
    ...
    // 1.10.0: read-only descent. Given a computed suspected dirty relative to
    // `sinceVer`, find a source in its cone whose version moved past sinceVer.
    // Prefers the underlying SIGNAL write (the true root cause) over an
    // intermediate computed. NEVER pulls -- uses only cached version fields, so
    // it cannot perturb the graph it is describing. DAG dep edges guarantee
    // termination (dep cycles are CycleError at eval, never persisted).
    function rootWrite(node, sinceVer) {
        let l = node.headDep;
        while (l !== null) {
            const dep = l.source;
            const moved = ((dep.version - sinceVer) | 0) > 0;
            if ((dep.flags & FLAG_COMPUTED) !== 0) {
                const deeper = rootWrite(dep, dep.evalVersion | 0);
                if (deeper !== null) return deeper;
                if (moved) return describeNode(dep);
            } else if (moved) {
                return describeNode(dep);   // signal: the root write
            }
            l = l.nextDep;
        }
        return null;
    }
    // 1.10.0: pull-side "why will this recompute / why did this fire". Applies
    // the EXACT predicate pullComputed uses -- ((dep.version - evalVer)|0) > 0
    // against the OBSERVER's evalVersion (not the dep's own evalVersion; that
    // was the rejected-sketch bug that reported every written signal). Also
    // surfaces deps pending through an intermediate computed, each tagged with
    // its root signal write. Empty for a clean computed and for non-computeds.
    function whyDirty(handle) {
        const node = liveNode(handle);
        if (node === undefined || (node.flags & FLAG_COMPUTED) === 0) return [];
        const reasons = [];
        const evalVer = node.evalVersion | 0;
        let link = node.headDep;
        while (link !== null) {
            const dep = link.source;
            let isReason = ((dep.version - evalVer) | 0) > 0;
            let root = null;
            if ((dep.flags & FLAG_COMPUTED) !== 0) {
                root = rootWrite(dep, dep.evalVersion | 0);
                if (root !== null) isReason = true;   // pending through this computed
            }
            if (isReason) {
                const d = describeNode(dep);
                if (root !== null && root.id !== dep.id) d.rootCause = root;
                reasons.push(d);
            }
            link = link.nextDep;
        }
    ...
    const registry = {signal, computed, effect, signalBox, computedBox, dispose, batch, flush, untrack, createRoot, getOwner, runWithOwner, createScope, onCleanup, stats, destroy, isTracking, hasObservers, observeObservers, forEachObserver, forEachSource, forEachOwned, ownerOf, nodeId, describe, whyDirty, onSettled, onGraphMutation};
    // 1.9.0: `using r = createRegistry()` destroys at block exit.
...
/**
 * Capture the currently-active owner as an opaque, gen-stamped handle
 * (undefined outside any effect/computed body). Companion to
 * {@link runWithOwner}; safe to hold across async boundaries via the
 * NODE_GEN ABA guard.
 */
...
/**
 * Run `fn` with `ownerHandle` as the current lifecycle owner. Stale, null,
 * undefined, or non-tracker handles degrade to rooted execution.
 */
...
export function whyDirty(handle) {
    return defaultRegistry.whyDirty(handle);
}
