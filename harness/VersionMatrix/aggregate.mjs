...
for (const w of manifest.workloads) {
    ...
    const spreadPct = (Math.max(...S.map((s) => s.frame.avg)) - Math.min(...S.map((s) => s.frame.avg))) / med.frame.avg * 100;
    ...
    med.evidence = { spreadPct: +spreadPct.toFixed(1), captureSpanMinutes: +spanMin.toFixed(1) };
    ...
    const warn = spreadPct > 15 ? '  !! SPREAD -- host unstable or mixed sessions; recapture before gating'
               ...
               : '';
    console.error(`  ${w.padEnd(20)} median frame.avg=${med.frame.avg.toFixed(4)} p99=${med.frame.p99.toFixed(4)} (n=${med.reps}, avg-spread ${spreadPct.toFixed(1)}%)${warn}`);
}
