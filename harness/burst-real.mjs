// burst-real.mjs -- reproduces Andrii's "layered burst flush warm" EXACTLY
// (pickLayeredSources strided edges, staticFraction=1, per-batch pull reads,
// updatesPerIteration=16) and contrasts it with burst-dag.mjs's contiguous
// window, on whatever engine is passed. Answers: redundancy or locality? and
// does the strided topology cost more than the contiguous one (the locality tell)?
//
//   node --expose-gc burst-real.mjs <Signal.js>
...
function build(r, pick) {
    ...
    for (let L = 0; L < P.layers; L++) {
        ...
        for (let j = 0; j < P.width; j++) {
            ...
            layer[j] = r.computed(() => {
                let s = 0;
                for (let k = 0; k < deps.length; k++) s += deps[k]();
                return s;
            });
        }
        prev = layer;
    ...
    const write = (i) => {
        const t0 = i * P.burst;
        for (let u = 0; u < P.burst; u++) {
            const t = t0 + u, sd = t % P.sources;
            sources[sd].set(t + sd);
        }
    };
    const readAll = () => {
        let s = 0;
        for (let e = 0; e < leaves.length; e++) s += leaves[e]();
        return s;
    };
...
function cap() {
    ...
    return {
        maxNodes: Math.ceil(nodes * 1.15),
        maxLinks: Math.ceil(links * 1.15),
        prealloc: "eager",
...
function structure(pick) {
    ...
    r.batch(() => g.write(1000));
    g.readAll();                     // ONE measured burst
    off();
    let maxRan = 0;
    for (const c of ran.values()) if (c > maxRan) maxRan = c;
    const st = r.stats();
    r.destroy();
    return {
        passes,
        recomputed: ran.size,
        maxRecompute: maxRan,
        flushPasses: st.flushPasses,
        poolGrowths: st.poolGrowths
    };
}
...
function timing(pick) {
    ...
    return {
        ...
        perBurstUs: (times[times.length >> 1] / P.iters) * 1000,
        minMs: times[0],
        _sink: sink
    };
}

for (const [label, pick] of [["contiguous (burst-dag)", pickContiguous], ["strided  (Andrii real)", pickLayered]]) {
