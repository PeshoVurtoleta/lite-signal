...
On the 1.10.0 engine `npm test` reports **559 tests, 558 pass, 0 fail, 1 skip**, and
`npm run test:gc` (Tier 2, `--expose-gc`) reports **567 tests, 566 pass, 0 fail, 1
...
- **`25-devtools-real-boot.test.mjs`** -- the devtools pairing contract (20 tests). Boots the REAL installed `@zakkster/lite-devtools` 1.8.0 against this engine through an import-rewrite rig (both share ONE engine instance -- the rig catches the two-instance failure mode where the package resolves to the published build instead of the local engine). Pins the live-probed ground truth: all 25 function exports + the `VERSION` const, the EXACT 16-key `capabilities()` fingerprint (`names`/`whyDirty` finally read true from devtools' own behaviour probes; `settled` reads **false** -- this engine's validation rejects the unknown key, so devtools fails closed, and the VALUE rather than the key set discriminates this pairing from 1.11.0), the named-node flow (`inspect().name` absent-not-empty on unnamed handles, serialize v2 name round-trip, `toDot`/`toTree` label precedence resolver > engine name > kind#id), the `whyDirty`/`explain` non-perturbing passthroughs, `watchSettled`'s documented fail-closed null, the `burstProfile()` live-handle and `stop()` summary shapes, and the `Symbol.dispose` stamps on devtools' own stopper handles.
