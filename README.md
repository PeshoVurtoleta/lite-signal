...
On the independent [js-reactivity-benchmark](https://github.com/volynetstyle/js-reactivity-benchmark) (Andrii Volynets' fork; 15 reactive libraries, 47 tests), `lite-signal` **1.9.0-preview** places **3rd overall by geomean (95.4 ms)** -- up from 4th on every sweep before 1.8.0, clearing 4th-place @reactively (136.0) by ~43% and 5th-place Preact Signals (148.8) by ~56%, behind only alien-signals (77.5) and reflex (89.7). The Andrii log is byte-identical to 1.8.0's: 1.9.0-preview's engine is sha256-identical over 14 propagation / flush-scaffold functions and differs only in `effect` and `createScope` by one existence-guarded `[Symbol.dispose]` stamp line each, so the ranking claim carries forward with zero re-verification cost.
...
`lite-signal` finishes **top-3 on 29 of the 47 tests** and is the **outright fastest of all 15** on **twenty shapes** -- the eight canonical propagation shapes (`avoidablePropagation`, `broadPropagation`, `deepPropagation`, `diamond`, `mux`, `repeatedObservers`, `triangle`, `unstable`), all four wide fan-in / fan-out shapes (`manyEffectsFromOneSource`, `manySourcesIntoOneComputedEffect`, `manySourcesIntoOneComputedEffectWithDirect`, `manySourcesIntoOneDirectEffect`), `molBench` (the mixed-app graph the suite uses as its realistic-workload proxy), and seven of the rectangular / layered DAG shapes (`64x6`, `24x8`, `120x7`, `40x10`, both `256->512x32 layered-dag` variants, and `10x10 dynamic component`). These are the aggregation and dynamic-DAG patterns that dominate live dashboards, scoreboards, and HUDs. The two engines ahead of it (alien-signals and reflex) are both push-eager designs that allocate on the hot path; `lite-signal` is the only top-3 finisher that allocates **nothing** in steady state. (Note: this suite measures reactivity *libraries* -- Vue's reactivity core, MobX, Solid, Preact Signals, etc. -- not full UI frameworks like React or Angular.)
...
### Named nodes + `whyDirty` (1.10.0)

Two cold-path diagnostics the profilers consume directly. Both are read-only and cost nothing when you don't ask -- the hot propagation bodies are byte-identical to 1.9.0.

```ts
const hp = signal(100, { name: "hp" });          // {name} on signal / computed / effect
const shield = computed(() => hp() > 0, { name: "alive" });
describe(hp).name;                                 // "hp"  (absent for unnamed nodes)

hp.set(80);
whyDirty(shield);   // [ { id, kind: "signal", name: "hp", value: 80 } ]
```
...
Honest numbers, against the same workload, with anti-DCE sinks and verified effect execution. All measurements: **Node 26.3.1, Apple M4 Pro (darwin/arm64)**, **one engine per cold process**, median across reps, machine-stamped `#STAMP` (engine + harness sha256, live registry config, host, node) on every output. Newer/faster machines shift all libs up proportionally; the relative ordering between libs is what matters -- and on this newer silicon the run-to-run noise floor is lower, so the sub-percent parity band tightens and the honest wins/losses stand out cleanly. Numbers below are the **fresh 1.9.0-preview bench sweep** on the v3 **microscope** (`benchmark.mjs`, six first-party shapes, default `prealloc: "eager"`) vs alien-signals -- the harness reports **median execution time + transient heap** rather than ops/s. 1.8.0 -> 1.9.0-preview changes the engine at exactly two sites: `effect` and `createScope` each gain one existence-guarded `[Symbol.dispose]` stamp line on the cold creation path (`typeof Symbol.dispose === "symbol"` resolved once as `SYMBOL_DISPOSE` at module load, then one `!== null` guard per stamp site). sha256 over the 14 other propagation / flush-scaffold function bodies is byte-identical between pristine 1.8.0 and pristine 1.9.0-preview (`pullComputed`, `executeEffect`, `markDownstream`, `flushEffects`, `allocateLink`, `severTail`, `createNode`, `runCleanup`, `disposeNode`, `createRoot`, `flush`, `batch`, `signal`, `computed`). Full outputs: [`bench/r.txt`](./bench/r.txt) (this microscope aggregate across four engines) and [`bench/rb.txt`](./bench/rb.txt) (the mirror sweep -- Andrii's canonical adapter verbatim, 47 rows, lite vs alien on the same M4 Pro host).

| Scenario   | What it stresses                | lite-signal | alien-signals | lite vs alien | transient heap (lite / alien) |
| ---------- | -------------------------------- | ----------- | ------------- | ------------- | ----------------------------- |
| **MUX**    | 256 signals -> 1 sum -> 1 effect (fan-in) | **22.98 ms** | 35.32 ms | **+34.9% faster** | **0 KB / 781 KB** (100% less) |
| **SELECTIVE DAG** | sqrt-layered, set churn, 2 read/iter | **2024 ms** | 2558 ms | **+20.9% faster** | **7.7 MB / 76.5 MB** (89.9% less, 9.9x) |
| **DYNAMIC DAG** | sqrt-layered, FAN=6, read flips each iter | **3737 ms** | 4453 ms | **+16.1% faster** | **3.4 MB / 55.7 MB** (94.0% less, 16.6x) |
| **BROADCAST** | 1 signal -> 1000 effects (fan-out) | **444.47 ms** | 468.88 ms | **+5.2% faster** | ~0 KB / ~0 KB (both fit under GC floor) |
| **KAIROS**    | 1 signal -> 1000 computeds -> 1 effect | 525.27 ms | 439.61 ms | -19.5% | **23 KB / 802 KB** (97.1% less, 34.3x) |
| **DEEP CHAIN** | 256-deep computed chain -> 1 effect | 173.54 ms | **100.41 ms** | -72.8% | **0.5 KB / 1062 KB** (100% less, >2000x) |

*(lower time = faster; transient heap = average delta-heap per rep, lower = less GC pressure)*

**Reading the table:** lite-signal's time wins cluster exactly where its zero-GC design pays off -- the **allocation-heavy dynamic shapes** (**SELECTIVE DAG +20.9%**, **DYNAMIC DAG +16.1%**), where alien-signals churns the nursery and lite's object pool allocates near-nothing, plus **MUX +34.9%** (fan-in aggregation) and **BROADCAST +5.2%** (fan-out; narrowed from +9.4% on 1.8.0 but still on the win side of parity). These are the patterns that dominate live UI workloads under input churn: dashboards, scoreboards, HUDs, leaderboards. That is **4 wins of 6** on time; on this sweep the wins hold within noise of the 1.8.0 M4 Pro sweep despite the two `[Symbol.dispose]` stamp lines added at effect / createScope creation, which is what "one existence guard per cold creation path, nothing on the propagation hot path" looks like in the propagation numbers. The two losses:

- **DEEP CHAIN (-72.8%)** is the engine's structurally worst shape and the largest open performance gap it has. On a 256-deep pull chain alien's flatter representation wins outright (100 ms vs 174 ms). It is not closed and it is not going to be closed by tuning. Note the inversion: **lite loses the row on time by 73% while allocating >2000x less heap**.
- **KAIROS (-19.5%)** widened from -15.9% on 1.8.0: 1 source -> 1000 computeds has almost nothing to retrack, so the pooled representation pays its per-node indirection with no allocation saving to show for it. Consistent host-noise drift on the shape where lite is architecturally weakest.

On **allocation pressure** `lite-signal` wins **5 of 6** and is one to four orders of magnitude below alien on every shape where GC pressure exists at all: DEEP CHAIN 0.5 KB vs 1062 KB (>2000x less -- yes, on the shape where lite is behind on time), MUX 0 KB vs 781 KB, KAIROS 23 KB vs 802 KB (34.3x), DYNAMIC DAG 3.4 MB vs 55.7 MB (16.6x), SELECTIVE DAG 7.7 MB vs 76.5 MB (9.9x). On BROADCAST both engines are effectively zero. Against preact-signals and solid-signals the heap gap is even wider on the fan-in / fan-out family -- solid allocates ~17 MB on MUX and ~15 MB on SELECTIVE DAG where lite allocates 0 KB and 7.7 MB respectively (`bench/r.txt`). **`lite-signal`'s differentiated position is ALLOCATION, not raw propagation speed**: competitive-to-winning throughput with dramatically lower GC pressure, not "fastest dynamic-graph engine". That headline is the one that reproduces across hosts, sweeps, and third-party version bumps -- because the object pool is the mechanism, not a tuning knob.

The mirror-sweep companion ([`bench/rb.txt`](./bench/rb.txt)) reproduces the picture across Andrii's 47 shapes on the same M4 Pro host: lite runs **parity-to-behind alien on throughput** across the head-to-head suite, wins outright on **3 of the 47** (`1000x5 - 25 sources (wide dense)` +10.7%, `manySourcesIntoOneComputedEffectWithDirect` +31.5%, `manySourcesIntoOneComputedEffect` +30.2%; `molBench` dropped to -0.4% parity from the razor-thin +0.3% win on 1.8.0), weak on the deep/layered-burst family. That head-to-head result and the top-of-file "3rd of 15 with 20 outright wins" both live on the same Andrii log -- they are not contradictory: the outright-of-15 rankings look at each shape across every framework in the field, so shapes where alien beats lite by 20-30% but reflex beats both by another 10% and everyone else is slower can still count as a lite outright win when the leaderboard is measured that way.
...
On the 1.11.0 engine `npm test` reports **580 tests, 579 pass, 0 fail, 1 skip**, and
`npm run test:gc` (Tier 2, `--expose-gc`) reports **588 tests, 587 pass, 0 fail, 1
...
- **`25-devtools-real-boot.test.mjs`** -- the devtools pairing contract (22 tests). Boots the REAL installed `@zakkster/lite-devtools` 1.8.0 against this engine through an import-rewrite rig (both share ONE engine instance -- the rig catches the two-instance failure mode where the package resolves to the published build instead of the local engine). Pins the live-probed ground truth: all 25 function exports + the `VERSION` const, the EXACT 16-key `capabilities()` fingerprint with **every flag true** -- the first engine on which devtools' full capability vector lights up, `settled` included (on the 1.10.0 pairing the same key set carries `settled:false`; the VALUE is the discriminator) -- the named-node flow (`inspect().name` absent-not-empty on unnamed handles, serialize v2 name round-trip, `toDot`/`toTree` label precedence resolver > engine name > kind#id), the `whyDirty`/`explain` non-perturbing passthroughs, the **`watchSettled` live battery** (one `{settles, ts}` per top-level clean drain, batches coalesce, empty flushes silent, `count()`/idempotent `stop()`/`Symbol.dispose`, fail-closed null on a non-settled registry, throwing consumer callback swallowed) plus the **mid-fire `stop()` interplay pin** (a self-stopping handle never costs a sibling handle its settle -- devtools' deferred detach composed with the engine's fixed fire loop), the `burstProfile()` live-handle and `stop()` summary shapes, and the `Symbol.dispose` stamps on devtools' own stopper handles.
...
- **`37-devtools-zerogc-probe.test.mjs`** -- the zero-GC claim on the reactive-benchmark weak-group shapes (5 tests), through both the engine's `stats().poolGrowths` counter and devtools 1.8.0's `watchAllocations()` feed: the pool never grows its backing store (`poolGrowths` delta == 0).
...
- a module-scoped int32 JIT sink advanced (the work loops were not optimised away), and
- **value-correctness**: a single `Int32Array` shadow oracle -- allocated ONCE before the loop, a rotating fixed window checked per tick (no per-tick allocation) plus a full sweep at teardown -- confirms every writable signal read back its last written value.
...
<details>
<summary>Stable vs dynamic topologies; Andrii Volynets' matrix, the 1.1.4 result, the 1.9.0-preview ranking, and the roadmap.</summary>
...
**1.9.0-preview on the official [js-reactivity-benchmark](https://github.com/volynetstyle/js-reactivity-benchmark) (15 libraries, 47 tests, raw log at [`bench/AndriiVolynetsReactiveBench.log`](./bench/AndriiVolynetsReactiveBench.log)):** `lite-signal` holds **3rd overall by geomean (95.4 ms)** -- the position it moved to on 1.8.0 -- behind only alien-signals (77.5, the field leader at 0.81x) and reflex (89.7, 0.94x), and **ahead of 4th-place @reactively (136.0, 1.43x) and 5th-place Preact Signals (148.8, ~56%)**. It finishes **top-3 on 29 of 47 tests** and is the **outright fastest of all 15 on twenty shapes** -- the eight canonical propagation shapes (`avoidablePropagation`, `broadPropagation`, `deepPropagation`, `diamond`, `mux`, `repeatedObservers`, `triangle`, `unstable`), all four wide fan-in / fan-out shapes (`manyEffectsFromOneSource`, `manySourcesIntoOneComputedEffect`, `manySourcesIntoOneComputedEffectWithDirect`, `manySourcesIntoOneDirectEffect`), `molBench`, and seven of the rectangular / layered DAG shapes. The Andrii log is byte-identical to 1.8.0's; 1.9.0-preview's engine differs from 1.8.0 only in `effect` and `createScope` by one existence-guarded `[Symbol.dispose]` stamp line each (cold creation path), so the ranking carries with zero re-verification cost. It remains the only object-pooled, zero-GC engine in the field.

#### 1. Stable Topologies (Fan-in / Fan-out / Broadcast)
In stable environments (game engines, particle systems, visualizers), `lite-signal` is blisteringly fast and maintains a near-zero allocation profile, keeping frame times perfectly flat -- unchanged through 1.1.4 and holding on 1.9.0-preview.
...
*1.9.0-preview (default eager) on the v3 microscope (`bench/r.txt`, Apple M4 Pro darwin/arm64, Node 26.3.1, one engine per cold process -- compare within-column, lite vs alien):*
| Scenario | alien-signals | lite-signal (1.9.0-preview) | result |
| :--- | :--- | :--- | :--- |
| **MUX** (256 sigs -> sum -> effect)           | 35.32 ms   | 22.98 ms   | **lite +34.9%** |
| **SELECTIVE DAG** (sqrt-layered, set churn)   | 2558 ms    | 2024 ms    | **lite +20.9%** |
| **DYNAMIC DAG** (sqrt-layered, FAN=6)         | 4453 ms    | 3737 ms    | **lite +16.1%** |
| **BROADCAST** (1 -> 1000 effects)             | 468.88 ms  | 444.47 ms  | **lite +5.2%** (narrowed from +9.4% on 1.8.0) |
| **KAIROS** (1 -> 1000 computeds)              | 439.61 ms  | 525.27 ms  | alien +19.5% (widened from -15.9% on 1.8.0) |
| **DEEP CHAIN** (256-deep pull)                | 100.41 ms  | 173.54 ms  | **alien +72.8%** |

> **Honest note (1.9.0-preview isolated run):** measured one-engine-per-process, lite-signal's
> wins cluster on the **allocation-heavy** dynamic shapes and fan-in / fan-out (MUX +34.9%,
> SELECTIVE DAG +20.9%, DYNAMIC DAG +16.1%, BROADCAST +5.2%) -- exactly where alien churns
> the nursery and lite's pool allocates near-nothing. The wins hold within noise of the 1.8.0
> M4 Pro sweep despite the two `[Symbol.dispose]` stamp lines added at effect / createScope
> creation; every other propagation function is sha256-identical between the two engines.
> **BROADCAST narrowed** from +9.4% to +5.2%; **KAIROS widened** from -15.9% to -19.5% --
> host-noise drift on the shape where lite is architecturally weakest, not an engine change.
> lite remains one to four orders of magnitude below alien on transient heap on every shape
> where GC pressure exists at all (see [`bench/r.txt`](./bench/r.txt)).

The mirror sweep ([`bench/rb.txt`](./bench/rb.txt), Andrii's canonical adapter verbatim, isolated-per-row, 10 reps, lite vs alien across all 47 shapes on the same M4 Pro host) reproduces the same picture: lite runs **parity-to-behind alien on throughput** across the head-to-head suite, wins outright on **3/47** (`1000x5 - 25 sources (wide dense)` +10.7%, `manySourcesIntoOneComputedEffectWithDirect` +31.5%, `manySourcesIntoOneComputedEffect` +30.2%; `molBench` dropped from a razor-thin +0.3% on 1.8.0 to -0.4% parity), weak on the deep/layered-burst family. Every row carries a `#STAMP` and the counters (`nodesRecomputed` / `edgesTraversed` / `sinkReads`) match Andrii's published suite exactly, so a lite-vs-alien delta here is identical work, not DCE. The retracking is verified correct by `retracking.difftest.mjs` -- 20,000 direct + 10,000 batched writes, 0 disagreements against the **published 1.1.5** reference.

**The Takeaway:** as of 1.1.4 you no longer have to choose, and 1.9.0-preview holds the line -- the engine ranks **3rd of 15** on the official js-reactivity-benchmark (the position it moved to on 1.8.0) and is the only zero-GC library in the field. `lite-signal` keeps the zero-GC, flat-arena profile for 120fps Canvas/WebGL **and** wins decisively on the high-churn dynamic and fan-in topologies that dominate live UI -- the shapes where zero allocation pays off most. BROADCAST narrowed on the M4 Pro sweep this cycle (+9.4% -> +5.2%) but stayed on the win side of parity; KAIROS widened (-15.9% -> -19.5%). The one shape where alien's flatter representation still leads decisively is the 256-deep computed pipeline (DEEP CHAIN, -72.8% on the M4 Pro isolated run) -- and that gap is structural, not a tuning bug.
