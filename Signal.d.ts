 ...
 *
 * 1.9.0 adds explicit-resource-management (`using` / `Symbol.dispose`) on
 * lifecycle objects. The engine is otherwise 1.8.0: same exports, same runtime
 * surface. The only type-level delta is the `[Symbol.dispose]()` slot the 1.9.0
 * engine stamps on every disposable handle -- both box prototypes, the
 * `Registry`, and every `Dispose` function (effect / `.subscribe()` /
 * `createScope`). Callable value handles (`Signal` / `Computed`) are NOT
 * stamped, matching the engine. See {@link ExplicitDisposeSlot} for how the
 * slot degrades on TypeScript `lib` targets that predate `Symbol.dispose`.
 */

/**
 * The `[Symbol.dispose]()` method the 1.9.0 engine stamps on every disposable
 * handle. Expressed so it never forces a `lib` upgrade: when the ambient
 * TypeScript `lib` knows `Symbol.dispose` (`esnext.disposable` -- required to
 * use `using` at all) this resolves to `{ [Symbol.dispose](): void }` and the
 * `using` declaration typechecks; on any older `lib` the key is `never`, the
 * member vanishes, and these types stay usable as plain values. The runtime
 * mirrors this exactly -- `Signal.js` feature-detects `Symbol.dispose` and only
 * stamps the slot when the host provides it (Node 20+ for manual
 * `[Symbol.dispose]()`, Node 24+ for `using`).
 */
export type ExplicitDisposeSlot = {
    [K in typeof Symbol extends { readonly dispose: infer S } ? S : never]: () => void;
};
...
/**
 * Idempotent dispose handle returned by {@link effect}, `.subscribe()`, and the
 * scope disposer of {@link Registry.createScope}. Callable as a plain function;
 * from 1.9.0 it ALSO carries `[Symbol.dispose]` (routing to the same idempotent,
 * gen-guarded disposer), so `using stop = effect(...)` disposes at block exit.
 */
export type Dispose = (() => void) & ExplicitDisposeSlot;

/**
 * Cleanup returned from an effect body (1.8.0, Reflex pattern A).
 * Runs before the next re-run and once on dispose, appended after any
 * `onCleanup(fn)` registered in the same body (they compose in call order).
 */
export type EffectCleanup = () => void;

/**
 * An effect body. Returning a function registers it as a cleanup (1.8.0);
 * any other return value is ignored. Computeds are NOT affected -- a computed
 * returning a function keeps treating that function as its value.
 */
export type EffectBody = () => void | EffectCleanup;
...
/** Method surface of {@link SignalBox}; see that type for the full contract. */
interface SignalBoxMembers<T> {
    /** Read the current value, tracking the read if inside an effect or computed. */
    get(): T;
 ...
 *
 * From 1.9.0 the shared prototype carries `[Symbol.dispose]` (-> `dispose(box)`),
 * so `using box = signalBox(0)` disposes the box at block exit.
 */
export type SignalBox<T> = SignalBoxMembers<T> & ExplicitDisposeSlot;

/** Method surface of {@link ComputedBox}; see that type for the full contract. */
interface ComputedBoxMembers<T> {
    /** Resolve the value, tracking the read if inside an effect or computed. */
    get(): T;
    /** Resolve the value WITHOUT tracking. */
    peek(): T;
 ...
 *
 * From 1.9.0 the shared prototype carries `[Symbol.dispose]` (-> `dispose(box)`),
 * so `using box = computedBox(fn)` disposes the box at block exit.
 */
export type ComputedBox<T> = ComputedBoxMembers<T> & ExplicitDisposeSlot;
...
export interface RegistryStats {
    ...
    poolGrowths: number;
    /**
     * Cumulative count of effect-flush drain passes. Advances once per pass, only
     * while a mutation-hook listener is attached (gated by the same `mutationHook !==
     * null` check that admits opcodes 6 and 7); frozen otherwise, so the steady-state
     * write path is byte-identical to 1.5.0. Combined with `totalAllocations` and
     * `poolGrowths`, lets devtools chart flush activity alongside allocation rate
     * without the engine itself computing rates. Reset by {@link Registry.destroy}.
     * Added in 1.6.0.
     */
    flushPasses: number;
}
...
interface RegistryMembers {
    ...
    createRoot<T>(fn: () => T): T;
    /**
     * Run `fn(dispose)` in a detached, untracked scope and return whatever `fn`
     * returns (1.6.0). The single `dispose` argument cascade-disposes every effect
     * and computed created inside `fn`. Lifecycle complement to {@link createRoot}:
     * `createRoot` only *detaches* (caller must dispose each child by hand),
     * `createScope` *adopts*, so one disposer tears down a subtree of unknown shape
     * -- the per-item lifecycle a keyed-list or scene reconciler needs. Plain
     * signals created directly in `fn` are NOT adopted (consistent with the 1.2.0
     * ownership rule); computeds and effects are. The owner never re-runs, so
     * direct-body reads in `fn` are untracked; reactive bindings belong in inner
     * effect/computed bodies. A scope created inside a consumer effect SURVIVES
     * that consumer's re-run -- the reconciler-critical detach property. The
     * disposer is the same gen-guarded, ABA-safe handle `effect()` returns and
     * resolves to the owner effect under `describe` / `nodeId` / `forEachOwned`.
     */
    createScope<T>(fn: (dispose: () => void) => T): T;
    /**
 ...
 *
 * From 1.9.0 the registry carries `[Symbol.dispose]` (-> `destroy`), so
 * `using r = createRegistry()` resets its pools at block exit.
 */
export type Registry = RegistryMembers & ExplicitDisposeSlot;
...
export function effect(fn: EffectBody, opts?: EffectOptions): Dispose;
...
/** Run `fn(dispose)` in a detached, adopting scope (1.6.0), bound to the default registry. See {@link Registry.createScope}. */
