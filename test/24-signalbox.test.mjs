...
test("box: Symbol.dispose disposes the box (1.9.0 `using` support)", () => {
    // The `using` declaration calls the object's [Symbol.dispose]() at block exit.
    // The syntax isn't parseable on every runtime, so this pins the underlying
    // method directly -- what `using sb = r.signalBox(...)` invokes. Covers the
    // [SYMBOL_DISPOSE] method installed on both box prototypes.
    const r = createRegistry();

    const sb = r.signalBox(5);
    const cb = r.computedBox(() => sb.get() * 2);
    assert.equal(sb.get(), 5);
    assert.equal(cb.get(), 10);

    // computedBox first (it depends on sb), then the signalBox.
    cb[Symbol.dispose]();
    sb[Symbol.dispose]();

    assert.equal(sb.get(), undefined, "signalBox is stale after Symbol.dispose");
    assert.equal(cb.get(), undefined, "computedBox is stale after Symbol.dispose");
    assert.equal(r.stats().signals, 0, "signal node returned to the pool");
    assert.equal(r.stats().computeds, 0, "computed node returned to the pool");

    // Idempotent: a second Symbol.dispose on the stale handle is a no-op, not a throw.
    sb[Symbol.dispose]();
    cb[Symbol.dispose]();
    r.destroy();
});
