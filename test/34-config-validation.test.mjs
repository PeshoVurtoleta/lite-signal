// createRegistry input validation (1.4.5 backport into 1.8.0-alpha.7).
//
...
describe("flushStrategy remains a first-class option (native validation preserved)", () => {
    it("a valid flushStrategy builds and passes the unknown-key gate", () => {
        for (const v of ["eager", "sab", "manual"]) {
            const r = createRegistry({ flushStrategy: v });
...
describe("createRegistry OOM rows are refused, not fatal (1.4.5 backport)", () => {
    const CHILD = join(HERE, "34-config-oom.child.mjs");
