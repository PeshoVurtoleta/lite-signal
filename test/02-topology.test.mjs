...
describe("self-referential signal write inside effect", () => {
    ...
    it("direct self-write terminates safely (no re-run semantics)", () => {
        const a = r.signal(0);
        let runs = 0;
        r.effect(() => {
            runs++;
        ...
        // Body sets a=1 once; markDownstream gates the self-loopback via
        // FLAG_COMPUTING, so the effect is not re-queued by its own write.
        assert.equal(runs, 1);
        assert.equal(a(), 1);
    });

    it("ping-pong write between two effects trips the flush pass guard", () => {
        const a = r.signal(0);
        const b = r.signal(0);

        // Both effects must be fully stabilized (FLAG_COMPUTING cleared) before
        // any flush sees them — otherwise the gate catches the loopback during
        // creation, same as the direct self-write case. A batch defers
        // flushEffects until both initial bodies have returned.
        assert.throws(() => {
            r.batch(() => {
                r.effect(() => { b.set(a() + 1); });
                r.effect(() => { a.set(b() + 1); });
