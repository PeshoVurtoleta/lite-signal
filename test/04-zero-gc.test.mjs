...
// Wall-clock budgets are unreliable under coverage instrumentation (c8/V8
// coverage adds 5-10x overhead). Detect it and keep the loop (for coverage)
// while not asserting a time budget that only fails because of the profiler.
const UNDER_COVERAGE = process.env.NODE_V8_COVERAGE != null;
...
describe("performance: deep chain pull cost", () => {
    it("256-deep chain pull stays cheap for 1k pulls (perf smoke; budget skipped under coverage)", () => {
        ...
        // Still ran the 1000 pulls above (deep-chain pull path is covered); only
        // the time budget is conditional.
        if (!UNDER_COVERAGE) {
