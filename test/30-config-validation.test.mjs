// createRegistry input validation (1.4.5 backport into 1.5.0).
//
// The full 25-config matrix from BRIEF_SIGNAL.md. Every malformed row must throw
// a TypeError whose message is prefixed `createRegistry: "<name>"` -- naming the
// bad OPTION (or the unknown KEY) -- and NO row may reach the internal
// `TypeError: ... reading 'nextFree'` that the unvalidated constructor produced
// on first use. Accepted rows are listed explicitly. The engine is otherwise
// frozen from 1.5.0-rc.2; only createRegistry's cold construction path changed.
...
// Regex from the brief: a throw that NAMES one of the five options or `config`.
const NAMED_OPTION = /^createRegistry: "(maxNodes|maxLinks|prealloc|onCapacityExceeded|maxFlushPasses|config)"/;
// A throw naming an UNKNOWN key (the typo the caller wrote).
const NAMED_UNKNOWN = /^createRegistry: "[^"]+" is not a recognized option/;
// The pre-fix failure this patch eliminates: the delayed internal allocator throw.
const NEXTFREE = /nextFree/;

// Matrix rows. `kind`:
//   "option"  -> throws a TypeError matching NAMED_OPTION
//   "unknown" -> throws a TypeError matching NAMED_UNKNOWN
//   "accept"  -> builds without throwing
const MATRIX = [
    ...
    for (const [name, cfg, kind] of MATRIX) {
        it(name + " -> " + kind, () => {
            if (kind === "accept") {
                ...
                // A built registry must actually work -- not one that dies on first use.
                const s = r.signal(1);
                assert.equal(s(), 1);
                return;
            }
...
describe("stats() population distinguishes eager from lazy (1.4.5 backport)", () => {
    it("eager populates the pool to capacity; lazy starts empty", () => {
        ...
        // stats() shape: 13 keys, no key removed.
...
describe("createRegistry OOM rows are refused, not fatal (1.4.5 backport)", () => {
    const CHILD = join(HERE, "30-config-oom.child.mjs");
