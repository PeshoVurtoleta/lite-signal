// Isolated OOM worker for test/30-config-validation.test.mjs.
//
// Run as: node --max-old-space-size=256 test/30-config-oom.child.mjs <case>
//
...
// Spawn-only worker. Under a bare `node --test` discovery every .mjs under test/
// is run with NO arguments; with no case to run, no-op cleanly (natural exit 0)
// and let the parent suite drive it via spawnSync with an explicit case. A
// provided-but-unknown case is still a hard error.
if (key !== undefined) {
    const cfg = CASES[key];
    if (cfg === undefined) {
