// test/30-throwing-equals.test.mjs -- a user `equals` that THROWS (1.7.0-alpha)
//
...
//
// SITE AUDIT (1.7.0 Signal.js). There are three LOGICAL equals sites, exercised
...
//   (a) signal set pre-check      :1188  eq(node.value, value)          (mirror :1209)
//   (b) batch revert check        :1195  eq(node.preBatchValue, value)  (mirror :1216)
//   (c) computed re-eval          :1059  eq(node.value, newValue)
// 1.7.0 ALSO ships signalBox, whose boxSet (Signal.js :1311-1350) mirrors the
// callable set path with its OWN two eq() calls (:1319 pre-check, :1326 revert;
// sab/manual mirror :1339 / :1346) -- so a raw grep shows eq() calls across both
...
// identically.
//
// Site (b) is DOCUMENTED, not asserted-as-atomic: the throw at :1195 happens
// AFTER node.value was written at :1194 but BEFORE the version bump/revert. We
...
describe("throwing equals (b): batch revert check", () => {
    ...
    // check at :1195 compares preBatchValue against the value on the SECOND set.
    // We craft equals to throw ONLY on that specific comparison (both operands
    // are the sentinel), so the pre-check at :1188 never throws.
    ...
    it("PINNED: throw propagates, value is the net (written) value, downstream fires", () => {
        ...
        try {
            r.batch(() => {
                s.set(X);        // :1195 compares (POISON, X) -> no throw
                s.set(POISON);   // :1195 compares (POISON, POISON) -> throws
            });
        } catch (e) { threw = e; }
        ...
        // node.value was written at :1194 before the throw -- it holds the net value.
        assert.equal(s.peek(), POISON, "PINNED: node.value is the value written at :1194 (the net value)");
...
describe("throwing equals (box): signalBox mirrors the callable set sites", () => {
    // signalBox's boxSet (Signal.js :1311-1350) invokes equals at two sites that
    // mirror the callable set path exactly: the pre-check at :1319 (before the
    // value is written -- a throw leaves the box unmutated) and the batch-revert
    // check at :1326 (after node.value is written at :1325 -- a throw strands the
    // version bump so downstream fires). Confirmed on 1.7.0: boxSet behaves
    ...
    it("(b-box) batch revert-check throw propagates, box holds the net value, downstream fires", () => {
        ...
        try {
            r.batch(() => {
                b.set(X);        // :1326 compares (POISON, X) -> no throw
                b.set(POISON);   // :1326 compares (POISON, POISON) -> throws
            });
        } catch (e) { threw = e; }

        assert.ok(threw instanceof Error && threw.message === "box-revert-boom",
            "the box revert-check throw must propagate out of the batch");
        // node.value was written at :1325 before the throw -- it holds the net value.
        assert.equal(b.peek(), POISON, "PINNED: box value is the value written at :1325 (the net value)");
