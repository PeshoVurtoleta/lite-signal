// test/30-throwing-equals.test.mjs -- a user `equals` that THROWS (1.8.0-alpha)
//
...
//
// SITE AUDIT (1.8.0 Signal.js). There are three LOGICAL equals sites, exercised
// through the CALLABLE signal/computed surface this file drives. 1.7+ splits .set
...
//   (a) signal set pre-check      :1245  eq(node.value, value)          (mirror :1266)
//   (b) batch revert check        :1252  eq(node.preBatchValue, value)  (mirror :1273)
//   (c) computed re-eval          :1116  eq(node.value, newValue)
// 1.8.0 ALSO ships signalBox, whose boxSet (Signal.js :1371-1420) mirrors the
// callable set path with its OWN two eq() calls (:1376 pre-check, :1383 revert;
// sab/manual mirror :1395 / :1403) -- so a raw grep shows eq() calls across both
...
// unmutated, and a throwing batch-revert propagates the net value with downstream
// firing -- verified to behave IDENTICALLY to the callable path. No behavioral
// divergence from 1.4.4 was observed on either path -- every assertion here holds
// identically.
//
// Site (b) is DOCUMENTED, not asserted-as-atomic: the throw at :1252 happens
// AFTER node.value was written at :1251 but BEFORE the version bump/revert. We
...
describe("throwing equals (b): batch revert check", () => {
    ...
    // check at :1252 compares preBatchValue against the value on the SECOND set.
    // We craft equals to throw ONLY on that specific comparison (both operands
    // are the sentinel), so the pre-check at :1245 never throws.
    ...
    it("PINNED: throw propagates, value is the net (written) value, downstream fires", () => {
        ...
        try {
            r.batch(() => {
                s.set(X);        // :1252 compares (POISON, X) -> no throw
                s.set(POISON);   // :1252 compares (POISON, POISON) -> throws
            });
        } catch (e) { threw = e; }
        ...
        // node.value was written at :1251 before the throw -- it holds the net value.
        assert.equal(s.peek(), POISON, "PINNED: node.value is the value written at :1251 (the net value)");
...
describe("throwing equals (c): computed re-eval", () => {
    ...
    // occurs on RE-eval. When it throws it is caught at :1121, cached as
    // FLAG_HAS_ERROR (:1124), re-thrown on every read until a dep change
...
describe("throwing equals (box): signalBox mirrors the callable set sites", () => {
    // signalBox's boxSet (Signal.js :1371-1420) invokes equals at two sites that
    // mirror the callable set path exactly: the pre-check at :1376 (before the
    // value is written -- a throw leaves the box unmutated) and the batch-revert
    // check at :1383 (after node.value is written at :1382 -- a throw strands the
    // version bump so downstream fires). Confirmed on 1.8.0: boxSet behaves
    ...
    it("(b-box) batch revert-check throw propagates, box holds the net value, downstream fires", () => {
        ...
        try {
            r.batch(() => {
                b.set(X);        // :1383 compares (POISON, X) -> no throw
                b.set(POISON);   // :1383 compares (POISON, POISON) -> throws
            });
        } catch (e) { threw = e; }

        assert.ok(threw instanceof Error && threw.message === "box-revert-boom",
            "the box revert-check throw must propagate out of the batch");
        // node.value was written at :1382 before the throw -- it holds the net value.
        assert.equal(b.peek(), POISON, "PINNED: box value is the value written at :1382 (the net value)");
