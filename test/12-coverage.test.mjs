...
// ---------------------------------------------------------------------------
// 1.6.0-alpha coverage closure.
//
// Four ported from the 1.4.0-rc pass -- branch gaps this engine shares with the
// 1.4.0 line (line numbers shifted): allocateLink dead-target (:338),
// executeEffect scheduler-reentrancy (:837), computed stale-handle read (:1126),
// and the disposeNode cursor-repair regression that removed the dead freeLink
// `-1` ternaries (:420). The 1.5.0 box/owner surface: box stale-handle guards
// (:1209/:1225/:1238), the computedBox `equals` option (:1293), and the
// top-level getOwner/runWithOwner delegators (:1888/:1896). New in 1.6.0: the
// two profiler opcodes on the flush path -- op 7 (effect enqueued, in
// markDownstream) and op 6 (flush pass, in flushEffects), both hook-gated. The
// provably-unreachable clamps (:388 link ledger, :1408 batchEpoch wraparound)
...
describe("disposeNode: cursor repair when a source dies under a parked cursor", () => {
    it("disposing a not-yet-retracked dep from the observer's own body does not corrupt the dep list", () => {
        ...
        // disposed source; opcode 4 fires only for a dep-set flip.
...
describe("mutation hook: flush-path opcodes 6 (flush pass) and 7 (effect enqueued)", () => {
    it("op 7 fires when an effect is queued and op 6 fires once per flush pass", () => {
        const seen = [];
        r.onGraphMutation((op, a1, a2) => { if (op === 6 || op === 7) seen.push([op, a1, a2]); });

        const s = r.signal(0);
        let effId = 0;
        const stop = r.effect(() => { effId; s(); });   // creation runs once, no flush yet
        seen.length = 0;                                  // ignore setup traffic

        s.set(1);   // marks the effect (op 7) then flushes it (op 6)

        const op7 = seen.filter((e) => e[0] === 7);
        const op6 = seen.filter((e) => e[0] === 6);
        assert.equal(op7.length, 1, "exactly one op-7 for the single enqueued effect");
        assert.ok(op7[0][1] > 0, "op 7 carries the effect node id");
        assert.ok(op6.length >= 1, "at least one op-6 flush-pass record");
        assert.ok(op6[0][1] >= 1 && op6[0][2] >= 1, "op 6 carries (passNo, queueLen), both >= 1");

        // Hook-gated: with no hook attached the counters and calls are skipped entirely.
        r.onGraphMutation(null);
        const before = r.stats().activeNodes;
        s.set(2);   // must still propagate, just silently
...
describe("signalBox / computedBox: stale-handle guards", {skip: !HAS_BOX}, () => {
    it("update / get / peek on a disposed box return undefined and no-op (no recycled-slot corruption)", () => {
        ...
        r.dispose(sb);
        sb.update((v) => v + 100);   // boxUpdate stale guard -> no-op
