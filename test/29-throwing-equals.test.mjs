// test/29-throwing-equals.test.mjs -- a user `equals` that THROWS (1.5.0-rc.1)
//
...
//
// SITE AUDIT (1.5.0 Signal.js). There are three LOGICAL equals sites, exercised
// through the CALLABLE signal/computed surface this file drives:
//   (a) signal set pre-check      :1096  eq(node.value, value)
//   (b) batch revert check        :1103  eq(node.preBatchValue, value)
//   (c) computed re-eval          :977   eq(node.value, newValue)
// 1.5.0 ALSO ships signalBox (new in 1.5.0), whose boxSet mirrors the callable
// set path with its OWN two eq() calls (:1201 pre-check, :1208 revert) -- so a
// raw grep shows five physical eq() invocations, but they collapse to the same
// three logical sites; the signalBox pair is byte-identical in logic to (a)/(b).
// signalBox is the headline feature of 1.5.0, so its two boxSet sites are pinned
// DIRECTLY below in describe "(box)": a throwing pre-check propagates unmutated,
// and a throwing batch-revert propagates the net value with downstream firing --
// verified to behave IDENTICALLY to the callable path. No behavioral divergence
// from 1.4.4 was observed on either path -- every assertion here holds identically.
//
// Site (b) is DOCUMENTED, not asserted-as-atomic: the throw at :1103 happens
// AFTER node.value was written at :1102 but BEFORE the version bump/revert. We
...
describe("throwing equals (b): batch revert check", () => {
    ...
    // check at :1103 compares preBatchValue against the value on the SECOND set.
    // We craft equals to throw ONLY on that specific comparison (both operands
    // are the sentinel), so the pre-check at :1096 never throws.
    ...
    it("PINNED: throw propagates, value is the net (written) value, downstream fires", () => {
        ...
        try {
            r.batch(() => {
                s.set(X);        // :1103 compares (POISON, X) -> no throw
                s.set(POISON);   // :1103 compares (POISON, POISON) -> throws
            });
        } catch (e) { threw = e; }
        ...
        // node.value was written at :1102 before the throw -- it holds the net value.
        assert.equal(s.peek(), POISON, "PINNED: node.value is the value written at :1102 (the net value)");
...
describe("throwing equals (c): computed re-eval", () => {
    ...
    // occurs on RE-eval. When it throws it is caught at :982, cached as
...
describe("throwing equals (box): signalBox mirrors the callable set sites", () => {
    // signalBox's boxSet (Signal.js :1197-1216) invokes equals at two sites that
    // mirror the callable set path exactly: the pre-check at :1201 (before the
    // value is written -- a throw leaves the box unmutated) and the batch-revert
    // check at :1208 (after node.value is written at :1207 -- a throw strands the
    // version bump so downstream fires). Confirmed on 1.5.0: boxSet behaves
    ...
    it("(b-box) batch revert-check throw propagates, box holds the net value, downstream fires", () => {
        ...
        try {
            r.batch(() => {
                b.set(X);        // :1208 compares (POISON, X) -> no throw
                b.set(POISON);   // :1208 compares (POISON, POISON) -> throws
            });
        } catch (e) { threw = e; }

        assert.ok(threw instanceof Error && threw.message === "box-revert-boom",
            "the box revert-check throw must propagate out of the batch");
        // node.value was written at :1207 before the throw -- it holds the net value.
        assert.equal(b.peek(), POISON, "PINNED: box value is the value written at :1207 (the net value)");
