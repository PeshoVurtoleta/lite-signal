// Real lite-devtools 1.8.0 boot against the 1.11.0 engine.
//
...
//
// GROUND TRUTH (probed live, devtools 1.8.0 x engine 1.11.0, 2026-09-06):
...
// cleanupReturn:T, names:T, whyDirty:T, settled:T }. `settled` reads TRUE here:
// devtools' throwaway-registry probe builds createRegistry({settled:true}) and
// verifies a default registry's onSettled throws -- both real on this engine.
// The SAME 16-key set with settled:F is the 1.10.0 pairing; the VALUE, not the
// key set, is the per-engine discriminator. watchSettled() hands back a LIVE
// { count, stop } on a consumer-built settled registry (one cb({settles, ts})
// per top-level, non-empty, clean drain; a batch of N writes is ONE settle) and
// the documented fail-closed null on a non-settled registry. Its stop() DEFERS
// the engine detach when called from inside its own callback -- devtools' guard
// against the (since-fixed) engine fire-loop sharp edge -- so a self-stopping
// consumer never costs a sibling handle a notification; pinned here as the
// composed cross-package contract. Named-node flow: inspect().name is ABSENT
// (not "") on unnamed handles; serialize() emits payload v:2 and round-trips
// name; toDot/toTree label precedence is labelResolver > engine name > kind#id.
// whyDirty()/explain() pass through non-perturbingly; explain() hops carry a
// reasons[] array (probed shape). burstProfile() live-handle and the 1.6.x
// Symbol.dispose stamps are unchanged.
...
describe("lite-devtools 1.8.0 boots against the 1.11.0 engine", () => {
    ...
    // 1.8 cleanupReturn, the 1.10.0 pair (names / whyDirty), AND the
    // 1.11.0-feature flag `settled` must ALL read true -- the first engine on
    // which devtools' full capability vector lights up. On the 1.10.0 pairing
    // the SAME 16-key set carries settled:false; the VALUE is the per-engine
    ...
    it("capabilities() fingerprints EXACTLY the 1.11.0 surface (16 keys, ALL flags on -- the first full-vector engine)", () => {
        ...
        // Present in 1.11.0.
        assert.equal(caps.owners, true, "1.11.0 has the owner tree");
        assert.equal(caps.mutationHook, true, "1.11.0 has onGraphMutation");
        assert.equal(caps.boxes, true, "1.11.0 has signalBox / computedBox");
        assert.equal(caps.roots, true, "1.11.0 has createRoot");
        assert.equal(caps.ownerCapture, true, "1.11.0 has getOwner / runWithOwner");
        assert.equal(caps.explicitDispose, true, "1.11.0 has explicit dispose");
        assert.equal(caps.poolPopulation, true, "1.11.0 stats expose pool population");
        ...
        assert.equal(caps.statsKeys, 14, "1.11.0 stats() has exactly 14 keys (13 + flushPasses)");
        ...
        // The 1.10.0 feature pair, capability-flagged by devtools 1.7.0.
        ...
        // The per-engine discriminator: TRUE here, FALSE on the 1.10.0 pairing.
        assert.equal(caps.settled, true,
            "onSettled ships in 1.11.0 -- devtools' throwaway {settled:true} probe must read true");
        // ...and the engine-side basis of that probe, pinned across the boundary:
        const scratch = SIG.createRegistry({ maxNodes: 16, maxLinks: 32, settled: true });
        assert.equal(typeof scratch.onSettled, "function",
            "a settled registry exposes a working onSettled -- half of devtools' behaviour probe");
        scratch.destroy();
        const plain = SIG.createRegistry({ maxNodes: 16, maxLinks: 32 });
        assert.throws(() => plain.onSettled(() => {}), /settled/,
            "a default registry's onSettled throws -- the other half of the probe (capability, not just a method)");
        plain.destroy();
    });
    ...
    it("burstProfile() is LIVE on 1.11.0: stop() summarizes a batched burst exactly", () => {
        ...
        assert.equal(DT.capabilities().burst, true, "precondition: 1.11.0 carries the burst payload");
        const bp = DT.burstProfile();
        assert.ok(bp !== null && typeof bp === "object", "burstProfile() must return a live handle on 1.11.0");
    ...
    it("track() registers a lifecycle listener against a 1.11.0-built handle", () => {
        const s = SIG.signal(0);
        const events = [];
    ...
    it("watchSettled() is LIVE on a consumer-built settled registry: exact coalesced accounting, non-perturbing attach", () => {
        // devtools 1.8.0's settle observer: one cb({settles, ts}) per top-level,
        // non-empty, clean drain; a batch of N writes is ONE settle; an empty
        // flush is none. The attach adds NO reactive observer (stats() holds).
        const r = SIG.createRegistry({ maxNodes: 64, maxLinks: 256, settled: true });
        const s = r.signal(0);
        r.effect(() => { s(); });

        const nodesBefore = r.stats().activeNodes;
        const payloads = [];
        const h = DT.watchSettled(r, (p) => payloads.push(p));
        assert.ok(h !== null && typeof h === "object", "a settled registry must yield a LIVE handle");
        assert.equal(typeof h.stop, "function");
        assert.equal(typeof h.count, "function");
        assert.equal(r.stats().activeNodes, nodesBefore,
            "non-perturbing contract: attaching watchSettled adds no reactive node");
        if (typeof Symbol.dispose === "symbol") {
            assert.equal(h[Symbol.dispose], h.stop, "the eighth stopper mirrors stop onto Symbol.dispose");
        }

        r.batch(() => { s.set(1); s.set(2); s.set(3); });   // ONE settle
        assert.equal(payloads.length, 1, "a 3-write batch coalesces to exactly ONE settle callback");
        assert.equal(payloads[0].settles, 1, "the payload carries the monotonic settle count");
        assert.equal(typeof payloads[0].ts, "number", "the payload carries a devtools-synthesized timestamp");
        r.flush();                                          // empty: no work queued
        assert.equal(payloads.length, 1, "an empty flush must not settle");
        s.set(9);                                           // ONE more
        assert.equal(payloads.length, 2, "each further clean drain settles once");
        assert.equal(h.count(), 2, "count() reports settles-since-attach");

        h.stop();
        s.set(10);
        assert.equal(h.count(), 2, "stop() freezes the count -- the listener is detached");
        h.stop();   // idempotent -- must not throw
        r.destroy();
    });

    it("watchSettled() fail-closes to null on a non-settled registry; a throwing consumer cb is swallowed", () => {
        // The documented null, never a live-but-dead handle.
        const plain = SIG.createRegistry({ maxNodes: 16, maxLinks: 32 });
        assert.equal(DT.watchSettled(plain, () => {}), null,
            "a default (non-settled) registry must yield the documented null");
        plain.destroy();

        // A throwing user callback is caught by devtools (cold/debug path):
        // the count still advances and the listener stays attached.
        const r = SIG.createRegistry({ maxNodes: 64, maxLinks: 256, settled: true });
        const s = r.signal(0);
        r.effect(() => { s(); });
        const h = DT.watchSettled(r, () => { throw new Error("consumer boom"); });
        s.set(1);
        s.set(2);
        assert.equal(h.count(), 2,
            "a throwing consumer callback must corrupt neither the count nor the listener");
        h.stop();
        r.destroy();
    });

    it("mid-fire stop() interplay: a self-stopping handle never costs a sibling handle its settle", () => {
        // The composed cross-package contract. devtools' stop() DEFERS the
        // engine detach when called from inside its own callback (its guard
        // against the engine's fire-loop mutation edge), and this engine's
        // fire loop additionally null-outs instead of splicing (the
        // 1.11.0-alpha.0 fix) -- belt and suspenders across the boundary. A
        // consumer stopping its own observer from inside a settle callback
        // must never silence the OTHER observer for that same drain.
        const r = SIG.createRegistry({ maxNodes: 64, maxLinks: 256, settled: true });
        const s = r.signal(0);
        r.effect(() => { s(); });
        ...
        const seen = [];
        let h1 = null;
        h1 = DT.watchSettled(r, () => { seen.push("first"); h1.stop(); });
        const h2 = DT.watchSettled(r, (p) => seen.push("second:" + p.settles));

        s.set(1);
        assert.deepEqual(seen, ["first", "second:1"],
            "the drain that watched the first handle stop itself must STILL deliver to the second");
        s.set(2);
        assert.deepEqual(seen, ["first", "second:1", "second:2"],
            "from the next drain on, only the surviving handle fires");
        h2.stop();
        r.destroy();
    });
